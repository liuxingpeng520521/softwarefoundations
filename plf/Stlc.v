...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | s_abs2 : forall T t1 t2 y,
      x<>y->
      substi s x t1 t2->
      substi s x <{\y:T, t1}> <{\y:T, t2}>
  | s_app : forall t1 t2 t3 t4,
      substi s x t1 t3->substi s x t2 t4->
      substi s x <{t1 t2}> <{t3 t4}>
  | s_tur :
      substi s x <{true}> <{true}>
  | s_fls :
      substi s x <{false}> <{false}>
  | s_test :forall t1 t2 t3 t4 t5 t6,
      substi s x t1 t4 ->substi s x t2 t5->substi s x t3 t6->
      substi s x <{if t1 then t2 else t3}> <{if t4 then t5 else t6}> 
.
...
Proof.
  ...
  +destruct (x0 =? s0)%string eqn:E. -rewrite eqb_eq in E. rewrite E. constructor. -apply s_var2. rewrite eqb_neq in E. assumption.
  +inversion H; subst. -assert ((s0 =? s0)%string=true). { rewrite eqb_eq. reflexivity. } rewrite H0. reflexivity. -assert ((x0 =? s0)%string=false). { rewrite eqb_neq. assumption. } rewrite H0. reflexivity.
  +apply s_app. -apply IHt1. reflexivity. -apply IHt2. reflexivity.
  +inversion H; subst. apply IHt1 in H2. apply IHt2 in H4. rewrite H2,H4. reflexivity.
  +destruct (x0 =? s0)%string eqn:E. -rewrite eqb_eq in E. rewrite E. constructor. -apply s_abs2. *rewrite eqb_neq in E. assumption. *apply IHt. reflexivity.
  +inversion H;subst. -assert ((s0 =? s0)%string=true). { rewrite eqb_eq. reflexivity. } rewrite H0. reflexivity. -assert ((x0 =? s0)%string=false). { rewrite eqb_neq. assumption. } rewrite H0. apply IHt in H5. rewrite H5. reflexivity.
  +inversion H;subst. reflexivity.
  +inversion H;subst. reflexivity.
  +apply s_test. -apply IHt1. reflexivity.  -apply IHt2. reflexivity.  -apply IHt3. reflexivity.
  +inversion H;subst. apply IHt1 in H3. rewrite H3. apply IHt2 in H5. rewrite H5. apply IHt3 in H6. rewrite H6. reflexivity.
Qed.  (** [] *)
      ...
      \x:Bool->Bool,
         \y:Bool->Bool,
            \z:Bool,
               (y (x z)) \in
      T.
Proof. exists <{(Bool->Bool)->(Bool->Bool)->Bool->Bool}>.
...
Example typing_nonexample_3 :
  ~ (exists S T,
        empty |--
          \x:S, x x \in T).
Proof.
  intros Hc. destruct Hc as [S Hc]. destruct Hc as [T Hc].
  inversion Hc; subst; clear Hc.
  inversion H4; subst; clear H4.
  inversion H5; subst; clear H5.
  inversion H2; subst; clear H2.
  rewrite H1 in H3. inversion H3.
  induction T2. +inversion H0.
