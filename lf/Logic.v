...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros. unfold not in H0. unfold not. intros. apply H0. apply H. apply H1. Qed.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intros [H0 H1]. apply H1. apply H0. Qed.
...
Theorem de_morgan_not_or : forall (P Q : Prop),
    ~ (P \/ Q) -> ~P /\ ~Q.
Proof.
  intros. unfold not in H. unfold not.
  split. +intros. apply H. left. apply H0.
...
Proof.
  ...
  -intros [H0|H1].
    ...
    +destruct H1. split. right. apply H. right. apply H0.
...
Proof.
  intros X P H.
  unfold not. intros [x NP]. apply NP. apply H.
Qed.
(** [] *)
...
Proof.
  intros X P Q. split.
  -intros [x [H0|H1]]. left. exists x. apply H0. right. exists x. apply H1.
  -intros [H0|H1].
   +destruct H0. exists x. left. apply H.
   +destruct H1. exists x. right. apply H.
Qed.
...
Proof.
  induction n.
  +intros. destruct H. rewrite H. simpl. reflexivity.
  +intros. destruct H. simpl in H. destruct m.
   -discriminate.
   -injection H as H0. rewrite H0. simpl. apply IHn. exists x. reflexivity.
  Qed.
...
Proof.
  ...
  -induction l as [|t l' IHl'].
    +simpl. intros [].
    +simpl. intros [H0|H1].
      *exists t. split. apply H0. left. reflexivity.
      *apply IHl' in H1. destruct H1 as [x0 [H10 H11]]. exists x0. split. apply H10. right. apply H11. 
...
Proof.
  ...
  -simpl. intros l' a. split.
   ...
   +intros [[]|H]. apply H.
  -simpl. intros l'0 a. split.
   + intros [H0|H1].
    *left. left. apply H0.
    *apply or_assoc. apply IH in H1. right. apply H1.
   +intros H. apply or_assoc in H. destruct H as[H0|H1].
    *left. apply H0.
    *apply IH in H1. right. apply H1.
Qed.
...
Proof.
  ...
  -induction l as [|t l' IHl'].
    ...
    *apply IHl'. intros x H0. apply H. right. apply H0.
  -induction l as [|t l' IHl'].
   +simpl. intros. destruct H0 as [].
   +simpl. intros [H0 H1]. intros x [H3|H4].
    *rewrite<- H3. apply H0.
    *apply IHl'. apply H1. apply H4.
Qed.   
...
Proof.
  intros Podd Peven n H0 H1. unfold combine_odd_even.
  destruct (odd n).
...
Proof.
  ...
  assert ( H: forall l1 l2:list X, rev_append l1 l2 = rev_append l1 [] ++ l2).
    { induction l1 as [|h' t'].
      - reflexivity.
      - simpl. rewrite IHt' with  (l2:=[h']). intros. 
        rewrite (IHt' (h'::l2)).
...
Proof.
  ...
  -rewrite even_S. destruct IHn'. destruct (even n').
...
Proof.
  ...
  -intros H. destruct b1.
    +simpl. split. reflexivity. apply H.
    +discriminate H.
  -intros H. destruct H. rewrite H. rewrite H0. simpl. reflexivity.
Qed.
Theorem orb_true_iff1 : forall x y,
  x || y = y || x.
Proof.
  intros.
  destruct x.
...
Proof.
  ...
  -intros. destruct H.
   ...
   +rewrite H. rewrite orb_true_iff1. reflexivity.
Qed.
...
Proof.
  intros x y. split.
  -intros. unfold not. intros. rewrite H0 in H. rewrite eqb_refl in H. discriminate H.
  -intros. unfold not. intros. destruct (x=?y) eqn:H0.
   +rewrite eqb_eq in H0. rewrite H0 in H. exfalso. apply H. reflexivity.
   +reflexivity.
Qed.
...
Proof.
  intros A eqb H. induction l1, l2.
  - simpl. split. 
    + intro e; reflexivity.
    + intro e; reflexivity.
  - simpl. split.
  ...
  - simpl. split.
    + intro e; discriminate e.
    + intro e; discriminate e.
  - simpl. destruct (eqb x x0) eqn: e.
    rewrite H in e. rewrite e. split.
    + intro tr. rewrite IHl1 in tr. rewrite tr. reflexivity.
    + intro eq. rewrite IHl1. injection eq. intro Hl; apply Hl.
    + split. 
      * intro eq; discriminate eq.
      * intro eq. exfalso. injection eq. intros _ xx.
        rewrite xx in e. simpl in e.
        assert (H2: eqb x0 x0 = true).
        { apply H. reflexivity. }
        rewrite H2 in e. discriminate e.
Qed.
...
Proof.
  intros X test. induction l as [|x0 l IHl].
  +simpl. split.
   -intro. reflexivity.
   -intro. reflexivity.
  +simpl. destruct (test x0). split.
   -intro. apply IHl in H. split. reflexivity. apply H.
   -intros [H0 H1]. apply IHl. apply H1.
   -split.
    *intro. discriminate H.
    *intros [H0 _]. discriminate H0.
Qed.
...
Theorem excluded_middle_irrefutable: forall (P : Prop),
  ~ ~ (P \/ ~ P).
Proof.
  intro. unfold not. intro H. apply H.
...
Proof.
  ...
  destruct H1.
  -apply H1.
  -unfold not in H0,H1. destruct H0. exists x. apply H1.
Qed. 
...
Proof. unfold excluded_middle ,peirce. intros. destruct H with P.
+apply H1.
+apply H0. intros. unfold not in H1. apply H1 in H2. destruct H2.
Qed.

Theorem classical2 :
  peirce->double_negation_elimination.
Proof. unfold double_negation_elimination ,peirce. intros. apply H with (False).
intros. unfold not in H0. apply H0 in H1. destruct H1. Qed.

Theorem classical3 :
  double_negation_elimination->de_morgan_not_and_not.
Proof. unfold double_negation_elimination , de_morgan_not_and_not. intros.
apply H. unfold not. intros. unfold not in H0. apply H0. split.
...
Proof. unfold excluded_middle , de_morgan_not_and_not. intros.
apply H. unfold not. intros. destruct H0.
apply H1. apply H0. Qed.

Theorem classical5: 
  excluded_middle -> implies_to_or.
Proof.
  unfold excluded_middle, implies_to_or. intros.
  unfold not.
  destruct H with P.
