...
Fixpoint type_check (Gamma : context) (t : tm) : option ty :=
  ...
  | <{case t0 of | inl x1 => t1 | inr x2 => t2}> =>
     ...
     | <{{T1+T2}}>=>
          ...
          if eqb_ty T3 T4 then return T3 else fail
     | _ =>fail
     end
  ...
  | <{t1 :: t2}> =>
      ...
      | <{{List T}}> =>
          T1<-type_check Gamma t1;;
          if eqb_ty T T1 then return <{{List T}}> else fail
      | _ =>fail
      end
...
Proof with eauto.
  ...
  fully_invert_typecheck Gamma t1 T1 T11 T12.
  remember (x31 |-> T11; Gamma) as Gamma'2.
    invert_typecheck Gamma'2 t2 T2.
    remember (x32 |-> T12; Gamma) as Gamma'3.
    invert_typecheck Gamma'3 t3 T3.
    case_equality T2 T3.
  (* lists (the [tm_lcase] is given for free) *)
  -eauto.
  -invert_typecheck Gamma t2 T2.
  ...
  (* let *)
  -  invert_typecheck Gamma t1 T1.
  remember (s |-> T1; Gamma) as Gamma'2.
  invert_typecheck Gamma'2 t2 T2.
  -(* fix *)
  fully_invert_typecheck Gamma t T1 T11 T12.
...
Fixpoint valuef (t : tm) : bool :=
  ...
  (* Complete the following cases *)
  (* sums *)
  | <{inl _ v}> | <{inr _ v}>=> if (valuef v) then true else false 
  | <{case _ of | inl _ => _ | inr _ => _}> =>false
  | <{nil _}> =>true
  | <{v1::v2}> => if andb (valuef v1) (valuef v2) then true else false
...
Fixpoint stepf (t : tm) : option tm :=
  ...
  | <{inl T2 t1}> =>
    match stepf t1,t1 with 
    | Some t1', _ => Some <{inl T2 t1'}>
    | _, _ => None
    end
  | <{inr T1 t2}> =>
    match stepf t2,t2 with 
    | Some t2', _ => Some <{inr T1 t2'}>
    | _, _ => None
    end
  | <{case t0 of | inl x1 => t1 | inr x2 => t2}>=>
  ...
  | <{t1.fst}> =>
    ...
    | None, <{(v1,v2)}> =>
      assert (valuef v1) (assert (valuef v2)
            (Some v1))
    | _,_ =>None
    end
  | <{t1.snd}> =>
...
Proof with auto.
  ...
  +constructor. apply IHt. destruct (valuef t0)...
  +constructor. apply IHt. destruct (valuef t0)...
  +constructor. -apply IHt1. destruct (valuef t1)...
    -apply IHt2. destruct (valuef t2)... destruct (valuef t1)...
  +constructor. -apply IHt1. destruct (valuef t1)...
  -apply IHt2. destruct (valuef t2)... destruct (valuef t1)...
Qed.
(** [] *)
...
Proof.
  intros. induction t; inversion H;try(constructor);subst...
  +simpl. apply IHt in H1. rewrite H1. reflexivity.
  +simpl. apply IHt in H1. rewrite H1. reflexivity.
  +simpl. apply IHt1 in H2. rewrite H2. apply IHt2 in H3. rewrite H3. reflexivity.
  +simpl. apply IHt1 in H2. rewrite H2. apply IHt2 in H3. rewrite H3. reflexivity.
  Qed.
...
Proof with auto.
  ...
  +apply IHt in H1. simpl. rewrite H1...
  +apply IHt in H1. simpl. rewrite H1...
  +apply IHt1 in H2. apply IHt2 in H3. simpl. rewrite H2,H3...
  +apply IHt1 in H2. apply IHt2 in H3. simpl. rewrite H2,H3...
Qed.
(** [] *)
...
Theorem complete_stepf : forall t t',
    t --> t'  ->  stepf t = Some t'.
Proof with auto. 
intros. generalize dependent t'. induction t;intros;inversion H;subst;simpl;
try(apply IHt in H3; rewrite H3); 
try(apply IHt1 in H3; rewrite H3); 
try(apply IHt1 in H4; rewrite H4);
try(apply IHt in H1; rewrite H1);
try(apply IHt1 in H6; rewrite H6);
try(remember H2 as H3; clear HeqH3; apply value_stepf_nf in H2; rewrite H2;
apply IHt2 in H4; rewrite H4; apply complete_valuef in H3; rewrite H3);
try(remember H6 as H7; clear HeqH7; apply value_stepf_nf in H6; rewrite H6;
apply complete_valuef in H7; rewrite H7);
try(remember H3 as H2; clear HeqH2; apply value_stepf_nf in H2; rewrite H2;
apply complete_valuef in H3; rewrite H3);
try(remember H4 as H5; clear HeqH5; apply value_stepf_nf in H4; rewrite H4;
apply complete_valuef in H5; rewrite H5);
try(remember H6 as H8; remember H7 as H9; clear HeqH8; clear HeqH9; apply value_stepf_nf in H6; rewrite H6; 
apply value_stepf_nf in H7; rewrite H7; apply complete_valuef in H8;rewrite H8; apply complete_valuef in H9; rewrite H9);
try(remember H2 as H3; remember H1 as H4; clear HeqH3; clear HeqH4; apply value_stepf_nf in H2; rewrite H2;
apply value_stepf_nf in H1; rewrite H1; apply complete_valuef in H3; rewrite H3; apply complete_valuef in H4; rewrite H4)...
Qed.



(*
+remember H3 as H2. clear HeqH2. apply value_stepf_nf in H2. rewrite H2.
        apply complete_valuef in H3. rewrite H3...
+apply IHt1 in H3. rewrite H3...
+remember H2 as H3. clear HeqH3. apply value_stepf_nf in H2. rewrite H2.
        apply IHt2 in H4. rewrite H4. apply complete_valuef in H3. rewrite H3...
...
+remember H6 as H7. clear HeqH7. apply value_stepf_nf in H6. rewrite H6.
apply complete_valuef in H7. rewrite H7...
+remember H6 as H7. clear HeqH7. apply value_stepf_nf in H6. rewrite H6.
apply complete_valuef in H7. rewrite H7...
+apply IHt1 in H3. rewrite H3...
...
apply IHt2 in H4. rewrite H4. apply complete_valuef in H3. rewrite H3...
+apply IHt1 in H6. rewrite H6...
+remember H6 as H8. clear HeqH8. remember H7 as H9. clear HeqH9. apply value_stepf_nf in H6. rewrite H6. apply value_stepf_nf in H7. rewrite H7.
apply complete_valuef in H8. rewrite H8. apply complete_valuef in H9. rewrite H9...
+apply IHt1 in H3. rewrite H3...
+remember H2 as H3. clear HeqH3. apply value_stepf_nf in H2. rewrite H2.
apply IHt2 in H4. rewrite H4. apply complete_valuef in H3. rewrite H3...
+apply IHt in H1. rewrite H1...
+remember H2 as H3. remember H1 as H4. clear HeqH3. clear HeqH4. apply value_stepf_nf in H2. rewrite H2.
apply value_stepf_nf in H1. rewrite H1. apply complete_valuef in H3. rewrite H3. apply complete_valuef in H4. rewrite H4...
+apply IHt in H1. rewrite H1...
+remember H2 as H3. remember H1 as H4. clear HeqH3. clear HeqH4. apply value_stepf_nf in H2. rewrite H2.
apply value_stepf_nf in H1. rewrite H1. apply complete_valuef in H3. rewrite H3. apply complete_valuef in H4. rewrite H4...
+apply IHt1 in H4. rewrite H4...
+remember H4 as H5. clear HeqH5. apply value_stepf_nf in H4. rewrite H4.
apply complete_valuef in H5. rewrite H5...
+apply IHt in H1. rewrite H1...
Qed.
*)
