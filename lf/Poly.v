...
Theorem app_assoc : forall A (l m n:list A),
  ...
  Proof.
    intros A l m n.
    simpl. induction l as [|k l' IHl'].
    -simpl. reflexivity.
    -simpl. rewrite->IHl'. reflexivity. Qed.

Lemma app_length : forall (X:Type) (l1 l2 : list X),
  length (l1 ++ l2) = length l1 + length l2.
  Proof. intros X l1 l2. induction l1 as [| n l1' IHl1'].
  - (* l1 = nil *)
    reflexivity.
  - (* l1 = cons *)
...
Definition filter_even_gt7 (l : list nat) : list nat:=
  filter even (filter (fun n=>7<=?n) l).
...
(** **** Exercise: 3 stars, standard (map_rev)
    ...
    auxiliary lemma. *)
Theorem map_l_n: forall (X Y:Type) (f:X->Y) (n:X) (l:list X),
    map f(l ++ [n])= map f (l)++[f n].
Proof.
  intros X Y f n l. induction l as [|m l'].
  -simpl. reflexivity.
  -simpl. rewrite->IHl'. reflexivity.
Qed.
...
Proof.
  ...
  -simpl. reflexivity.
  -simpl. rewrite<-IHl'. rewrite->map_l_n. reflexivity.
Qed.
