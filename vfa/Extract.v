...
Proof.
  intros a l S. induction S; simpl. (**leb:<=**)
  - constructor.
  - bdall; auto.
  ...
  - bdall; auto.
    ...
    + constructor; auto.
      ...
      unfold not in H0. unfold not in H1. auto. lia.
Qed.
...
Proof.
  ...
  - destruct (leb x a).
    ...
    + apply perm_trans with (a :: x :: l).
      * apply perm_swap. (**交换顺序**)
      * apply perm_skip. assumption. (**取消单个元素**)
Qed.
...
Proof.
  induction t; intros; bdall. 
  +exfalso. Search (_<_->False). apply Ztac.elim_concl_le in H0. apply H1.
  Search(_<=_->_<_\/_=_). apply Zle_lt_or_eq in H0. destruct H0;auto.
  apply Abs_inj in H0. subst. exfalso. apply H. reflexivity.
  + exfalso. apply Ztac.elim_concl_le in H2. apply H3.
   apply Zle_lt_or_eq in H2. destruct H2;auto.
  apply Abs_inj in H2. subst. exfalso. apply H. reflexivity.
  Qed.
(** [] *)
