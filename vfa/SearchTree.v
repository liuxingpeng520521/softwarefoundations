...
Proof.
   ...
   +auto.
   +bdestruct(k0<?k).
    -simpl. inv H. destruct H3. split;auto.
    -bdestruct (k0>?k).
     *simpl. inv H. destruct H4. split;auto.
     *simpl. inv H. destruct H4. split;auto.
Qed.
...
Proof.
  ...
  +inv H;subst. bdestruct (k <? k0).
   -constructor;simpl;auto. apply ForallT_insert;auto.
   -bdestruct (k >? k0).
    *constructor;simpl;auto. apply ForallT_insert;auto.
    *assert(k=k0). {lia. } subst;auto.
Qed. 
(** [] *)
...
Proof.
  ...
  +subst. repeat(rewrite lookup_insert_eq). reflexivity.
...
Proof.
  exists nat, 2 ,empty_tree, 1. simpl. intro. inversion H. Qed.
...
Proof.
  exists nat, 1,2 ,1,2,empty_tree. simpl.
  split; intro; inversion H. Qed.
...
Proof.
  ...
  induction t; simpl; intros.
  +inv H0.
  +inv H;subst. apply in_app_or in H0. simpl in H0.
  destruct H0 as [H0 | [H0|H0]].
   -bdestruct(k<?k0).
    *auto.
    *apply elements_preserves_relation with (k:=k) (v:=v) in H5 .
       lia. auto.
   -injection H0 as Hl Hr;subst. assert (k >? k = false). { bdestruct (k >? k); lia. }
   rewrite H. assert (k <? k = false). { bdestruct (k <? k); lia. }
   rewrite H0. auto.
   -bdestruct(k<?k0).
   ...
   *bdestruct(k>?k0).
    ++auto.
    ++apply elements_preserves_relation with (k:=k) (v:=v) in H6 . lia. auto.
Qed.
...
Proof.
  induction t;intros;simpl.
  +constructor.
  +inv H;subst. unfold list_keys in *. rewrite map_app.
  ...
  forall (V : Type) (v v0 : V) (e1 e2 : list (key * V)) (k k0 : key),
    ...
    if k <? k0 then
      (kvs_insert k v e1) ++ (k0,v0)::e2
           ...
           e1 ++ (k,v)::e2.
Proof.
  induction e1;induction e2;intros.
  +simpl. reflexivity.
  +simpl. reflexivity.
  +Admitted.
...
Proof.
  ...
  - auto.
  - simpl. bdall.
    + (* k<x *) rewrite kvs_insert_split.
      ++ bdall. rewrite IHBST1. auto.
      ++ apply elements_preserves_forall in H. unfold uncurry in H. auto.
      ++ apply elements_preserves_forall in H0. unfold uncurry in H0. auto.
    + (* k>x *) simpl. rewrite kvs_insert_split.
      ++ bdall. rewrite IHBST2. auto.
      ++ apply elements_preserves_forall in H. unfold uncurry in H. auto.
      ++ apply elements_preserves_forall in H0. unfold uncurry in H0. auto.
    + (* k=x *) simpl. rewrite kvs_insert_split.
      ++ bdall.
...
Proof.
  induction t;intros;simpl.
  +unfold Abs. simpl. unfold map_bound. simpl. reflexivity.
  +inv H;subst. unfold Abs in *. simpl in *. unfold map_bound.
   bdestruct (k0<? k).
    -rewrite elim_not_suitable_app_r with (P := fun x => x>= k).
     *apply IHt1. assumption.
     *constructor. lia. apply forall_transform in H5.
      apply forall_trans_pm. apply Forall_impl with (Q := fun x => x >= k)in H5.
      **apply H5.
      **lia.
     *lia.
...
Proof.
  ...
  - unfold map_bound.
    ...
    bdestruct (k =? k0).
    + assert (G1: k0 <? k = false). { bdestruct(k0 <? k); lia. }
      assert (G2: k0 >? k = false). { bdestruct(k0 >? k); lia. }
      rewrite G1, G2.
      reflexivity.
    + bdestruct (k0 <? k); bdestruct (k0 >? k).
      * exfalso. lia.
      * unfold union.
