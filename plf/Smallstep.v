...
Proof.
  ...
  - (* ST_Plus2 *) inversion Hy2; subst.
    + (* ST_PlusConstConst *)
      inversion Hy1.
    + (* ST_Plus1 *) inversion H; subst. inversion H3.
    + (* ST_Plus2 *)
...
Inductive step : tm -> tm -> Prop :=
  ...
  where " t '-->' t' " := (step t t').
Lemma double_neg : forall P : Prop, P -> (P->False)->False.
Proof.
  intros. apply H0. apply H. Qed.
Lemma value_not_same_as_normal_form :
  exists v, value v /\ ~ normal_form step v.
Proof.
  unfold normal_form. unfold not. exists (P (C 1) (C 1)).
  split.
  +apply v_funny.
  +apply double_neg. exists (C 2). apply ST_PlusConstConst.
Qed.
End Temp1.
...
Lemma value_not_same_as_normal_form :
  exists v, value v /\ ~ normal_form step v.
Proof.
  unfold normal_form. unfold not. exists (C 1).
  split.
  +apply v_const.
  +apply double_neg. exists (P (C 1) (C 0)). apply ST_Funny.
Qed.
...
Example bool_step_prop4_holds :
  bool_step_prop4.
Proof.
  unfold bool_step_prop4. apply ST_ShortCircuit. Qed.
...
Theorem step_deterministic :
      ~ deterministic step.
    Proof.
      unfold deterministic. unfold not. intros.
      assert (test tru tru tru = tru). 
      +apply H with (test (test tru tru tru) tru tru);
        repeat constructor.
      +inversion H0.
    Qed.
(*
...
Proof.
  ...
  +destruct H.
  exists (test x t2 t3). constructor. assumption.
Qed.
(* FILL IN HERE
...
(* remove ST_IfTrue. then (tif ttrue ttrue tfalse) fails strong_progress*)
*)
(* Do not modify the following line: *)
...
Lemma test_multistep_4:
  ...
  -->*
      P
        ...
        (C (2 + (0 + 3))).
Proof.
  eapply multi_step. { apply ST_Plus2. +apply v_const. + apply ST_Plus2. -apply v_const. -apply ST_PlusConstConst.  }
  eapply multi_step. { apply ST_Plus2.
                       - apply v_const.
                       - apply ST_PlusConstConst. }
  apply multi_refl.
...
Proof.
  ...
  + induction P21.
   -reflexivity.
   -unfold not in P12. exfalso. apply P12. exists y; assumption.
  + induction P21; subst.
   -exfalso. apply P22. exists y; assumption.
   - assert (y0 = y).
  {eapply step_deterministic; eauto. } subst.
...
Proof.
  ...
  +inversion H; subst. apply IHHs in H2. constructor; assumption.
...
Proof.
  ...
  induction H; subst.
  +apply nf_is_value in H0. inversion H0; subst. exists n.
  split. - reflexivity. -constructor.
  +apply IHmulti in H0. destruct H0. destruct H0.
  exists x0. split. -assumption. -eapply step__eval; eassumption.
Qed.
...
Theorem evalF_eval : forall t n,
  evalF t = n <-> t ==> n.
Proof.
  intros. split; generalize dependent n; induction t; intros; simpl in *; subst.
  +constructor.
  +constructor. -apply IHt1. reflexivity. -apply IHt2. reflexivity.
  +inversion H; subst. reflexivity.
  +inversion H; subst. apply IHt1 in H2. apply IHt2 in H4. rewrite H2,H4. reflexivity.
Qed. 
(** [] *)
...
Proof.
  ...
  +inversion H0. -reflexivity. -inversion H3. -inversion H4.
  +inversion H0; subst. -inversion H. 
   -assert(t1'=t1'0). {eapply IHstep. assumption. } rewrite H1. reflexivity. 
   -inversion H3; subst; inversion H.
  +inversion H1; subst. -inversion H0. -inversion H;subst;inversion H5. 
...
Theorem combined_strong_progress :
  ...
  \/ ~ (forall t, value t \/ (exists t', t --> t')).
Proof.
  right. intro. destruct H with (test (C 0) tru tru).
  +inversion H0.
  +destruct H0. inversion H0;subst. inversion H5.
Qed.
(** [] *)
  ...
  unfold par_loop. intros. destruct H.
     eapply multi_step.
       ...
       *eapply multi_step.
        ++apply CS_Par2. apply CS_IfStep.
          apply BS_Eq.
        ++rewrite H0. simpl. eapply multi_step.
          --apply CS_Par2. apply CS_IfTrue.
          --eapply multi_step.
            **apply CS_Par2. apply CS_SeqStep. apply CS_AsgnStep. apply AS_Plus1. apply AS_Id.
            **eapply multi_step.
              +++apply CS_Par2.  apply CS_SeqStep. rewrite H. apply CS_AsgnStep. apply AS_Plus.
              +++eapply multi_step.
...
Proof.
  ...
  +apply IHn in H. destruct H. destruct H. exists (X!->S n;x).
   split.
   -eapply multi_trans. *apply H. *apply par_body_n__Sn. apply H0.
   -split. 
    *rewrite t_update_eq. reflexivity.
    *rewrite t_update_neq. ++destruct H0. assumption. ++ unfold not. intros.
    inversion H1.
Qed.
(** [] *)
...
Proof.
  ...
  intros e. induction e; intros; simpl; try (eapply multi_step; constructor; apply multi_refl).
  +apply multi_trans with (s_compile e2 ++ [SPlus] ++ p, aeval st e1 :: stk).
    ...
    *apply IHe2.
    *apply multi_trans with (p, aeval st e1 + aeval st e2 :: stk).
     ++eapply multi_step. --constructor. --simpl. apply multi_refl.
     ++apply multi_refl.
  +apply multi_trans with (s_compile e2 ++ [SMinus] ++ p, aeval st e1 :: stk).
    ...
    *apply IHe2. *apply multi_trans with (p, aeval st e1 - aeval st e2 :: stk).
     ++eapply multi_step. --constructor. --simpl. apply multi_refl.
     ++apply multi_refl.
