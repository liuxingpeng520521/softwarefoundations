...
Proof.
  ...
  +inversion H;subst.
  -apply E_IfFalse.
   * simpl in H5. destruct (beval st b) eqn:E. 
    **discriminate.
    **reflexivity.
   *apply H6.
  - apply E_IfTrue.
...
Proof.
  intros b c Hb. split; intro.
  +apply while_true_nonterm with b c st st' in Hb. unfold not in Hb. apply Hb in H. inversion H.
  +apply while_true_nonterm with (c:=<{ skip }>) (st:=st) (st':=st') in Hb as Hbb.*)
Theorem while_true : forall b c,
  bequiv b <{true}>  ->
  cequiv
    <{ while b do c end }>
...
Theorem assign_aequiv : forall (X : string) (a : aexp),
  ...
  cequiv <{ skip }> <{ X := a }>.
Proof.
  intros. split; intro;unfold aequiv in H.
  +
  apply identity_assignment with (x:=(X)) in H0. 
  inversion H0; subst. rewrite H. apply E_Asgn.
  reflexivity.
  +
  apply identity_assignment with (x:=(X)).  
  inversion H0; subst. rewrite<-H. apply E_Asgn.
  reflexivity.
Qed.
...
Definition prog_b : com :=
  <{ if (X = 0) then
     ...
     X := X - Y;
     Y := 0 }>. (*X=0:X=0,Y=0/X>0:X=X,Y=0-->Y=0*)
...
Definition equiv_classes : list (list com):=
  ...
  [prog_b;prog_e];[prog_c;prog_h];[prog_i]].
...
Proof.
  unfold atrans_sound. intro. unfold aequiv. intro.
...
Proof.
  unfold btrans_sound. intro. unfold bequiv. intro.
  assert(H: forall a1, aeval st a1=aeval st (optimize_0plus_aexp a1)).
  {intros. rewrite<-optimize_0plus_aexp_sound. reflexivity. }
  induction b; simpl; try reflexivity;
  try( rewrite<-H with(a1:=a1); rewrite<-H with(a1:=a2); reflexivity).
...
Proof.
  unfold ctrans_sound. intro.
  induction c.
  +simpl. apply refl_cequiv.
  +simpl. apply CAsgn_congruence. apply optimize_0plus_aexp_sound.
  +simpl; apply CSeq_congruence; assumption.
  +simpl.  apply CIf_congruence.
   -apply optimize_0plus_bexp_sound.
   -apply IHc1.
   -apply IHc2.
  +simpl. apply CWhile_congruence.
   -apply optimize_0plus_bexp_sound.
   -apply IHc.
Qed.
...
Proof.
  unfold ctrans_sound. intro. unfold optimizer.
...
Theorem subst_equiv_property' : forall x1 x2 a1 a2,
  ...
  cequiv <{ x1 := a1; x2 := a2 }>
         <{ x1 := a1; x2 := subst_aexp x1 a1 a2 }>.
Proof. intros. split.
+intros. inversion H0;subst;clear H0. apply E_Seq with st'0.
 -assumption.
 -inversion H3;subst. inversion H6;subst. clear H3 H6. apply E_Asgn.
 assert( aeval (x1 !-> aeval st a1; st) a2 = aeval st (subst_aexp x1 a1 a2)).
 {induction a2;intros;simpl;
 try (unfold t_update; destruct (x1 =? x)%string);
 try rewrite IHa2_1, IHa2_2;
 reflexivity. } rewrite H0. clear H0. apply aeval_weakening.
 induction a2;simpl;try(constructor); try(assumption).
 destruct (x1 =? x)%string eqn :E. ++assumption. ++constructor. rewrite String.eqb_neq in E. assumption.
+intros. inversion H0;subst;clear H0. apply E_Seq with st'0.
 -assumption.
 -inversion H3;subst. inversion H6;subst. clear H3 H6. apply E_Asgn.
 assert( aeval (x1 !-> aeval st a1; st) a2 = aeval st (subst_aexp x1 a1 a2)).
 ...
 try (unfold t_update; destruct (x1 =? x)%string);
 try rewrite IHa2_1, IHa2_2;
 reflexivity. } rewrite H0. clear H0. symmetry. apply aeval_weakening.
 induction a2;simpl;try(constructor); try(assumption).
 destruct (x1 =? x)%string eqn :E. ++assumption. ++constructor. rewrite String.eqb_neq in E. assumption.
Qed.
...
Proof.
  ...
  { constructor. }
  intro. unfold cequiv in H0. apply H0 in H; clear H0. apply loop_never_stops in H.
  contradiction.
Qed.
(*·´Ö¤·¨
  unfold not. unfold cequiv. intro.
  remember <{ X := 1 }> as c.
  remember (X !-> 1) as st1.
  assert (H1 : empty_st =[ c ]=> st1).
  { rewrite Heqc, Heqst1. apply E_Asgn; reflexivity. }
  assert( H2:  cequiv
      <{ while true do c end }>
      <{ while true do skip end }>).
  {
    apply while_true. apply refl_bequiv.
  }
  assert(forall st st' : state,
  st =[ while true do skip end ]=> st' <->
  st =[ while true do c end ]=> st').
  { unfold cequiv in H2. symmetry. apply H2. }
  assert(forall st st' : state,
  st =[ skip ]=> st' <->
  st =[ while true do c end ]=> st').
  {intros. rewrite<-H. rewrite<-H0. reflexivity. }
  clear H2 H0.
  assert(empty_st =[ while true do c end ]=> st1).
  {apply E_WhileTrue with(st':=st1). 
    +simpl. reflexivity.
    +apply H1.
    +apply H3. apply E_Skip. }
  apply H3 in H0.
  assert(empty_st =[ skip ]=> empty_st). {apply E_Skip. }
  assert (Hcontra : st1 = empty_st).
  {apply (ceval_deterministic <{skip}> empty_st); assumption. }
  assert (Hcontra' : st1 X= empty_st X).
...
Proof.
  left.
  unfold cequiv. intros. split; intro.
  +inversion H; subst.
...
Proof. 
  ...
  assert(empty_st=[pcopy]=>(Y!->1;X!->0)).
  {apply H. apply E_Seq with (X!->0).
   +apply E_Havoc.
   +apply E_Havoc. }
  inversion H0; subst. inversion H3; subst. inversion H6; subst.
  simpl in H7. rewrite t_update_eq in H7.
  assert((Y!->n;X!->n) X=n).
  {rewrite t_update_permute. + rewrite t_update_eq. reflexivity. 
  +unfold not. intro. inversion H1. }
  assert((Y!->n;X!->n) Y=n).
  {rewrite t_update_eq. reflexivity. }
  rewrite H7 in H1. rewrite H7 in H2.
  rewrite t_update_permute in H1.
  +rewrite t_update_eq in H1. rewrite t_update_eq in  H2.
...
Lemma p1_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p1 ]=> st'.
Proof.
  unfold not. unfold p1. intros. remember (<{ while ~ (X = 0) do  havoc Y;  X := X + 1 end }> ) as p1.
  induction H0; inversion Heqp1; subst.
  +simpl in H0. apply H. apply negb_false_iff in H0. apply Nat.eqb_eq in H0. assumption.
  +apply IHceval2.
   -inversion H0_; subst. inversion H6; subst. rewrite t_update_eq. simpl. rewrite<-plus_n_Sm.
    intro. inversion H1.
   -reflexivity. 
Qed.
Lemma p2_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p2 ]=> st'.
Proof.
  unfold not. unfold p2. intros. remember (<{ while ~ (X = 0) do skip end }>) as p2.
  induction H0; inversion Heqp2;subst.
  +simpl in H0. apply H. apply negb_false_iff in H0. apply Nat.eqb_eq in H0. assumption.
  +inversion H0_; subst. apply IHceval2.
   - assumption.
   - reflexivity.
Qed.
...
Proof.
  ...
  +inversion H; subst.
   ...
   assert ((X !-> aeval st'1 <{ X + 1 }>;
   st'1) X<>0).
   {
    rewrite t_update_eq. simpl. rewrite<-plus_n_Sm. unfold not. intro. inversion H0.
   }
   apply p1_may_diverge  with (st':=st') in H0. contradiction.
  +inversion H; subst.
   -apply E_WhileFalse. assumption.
   -inversion H3; subst. simpl in H2. apply negb_true_iff in H2. apply Nat.eqb_neq in H2.
...
Proof. 
  unfold not. intro. unfold cequiv in H.
  assert((X!->1 ;empty_st)=[p4]=>(Z!->0;X!->0;Z!->1;X!->1)).
  {rewrite<-H. apply E_Seq with (Z!->1;X!->1).
   +apply E_Asgn. reflexivity. 
...
Proof. 
  split; intro.
  +unfold p5 in H. remember(<{ while X <> 1 do havoc X end }>) as p5.
   induction H; inversion Heqp5; subst.
   -simpl in H. apply negb_false_iff in H. apply Nat.eqb_eq in H.
   assert((X!->1;st)=st). {rewrite<-H. apply t_update_same. }
    rewrite<-H0 at 2. apply E_Asgn. reflexivity.
   -inversion H0; subst. assert ((X!->n;st) =[p6]=> st''). {apply IHceval2. reflexivity. }
    inversion H2; subst. simpl.  rewrite t_update_shadow.
    apply E_Asgn. reflexivity.
  +inversion H; subst. simpl in *.
   destruct(negb (st X=?1)) eqn: eqnat.
   -apply E_WhileTrue with ((X!->1;st)).
    *simpl. assumption.
    *apply E_Havoc.
    *apply E_WhileFalse. reflexivity.
   -remember eqnat as eqnat1. clear Heqeqnat1.
   apply negb_false_iff in eqnat.  apply Nat.eqb_eq in eqnat. rewrite<-eqnat.
    rewrite t_update_same.
    apply E_WhileFalse. simpl. assumption.
Qed.
...
Proof.
  ...
  +inversion H2; subst. inversion H5; subst. inversion H8; subst.
    assert ((l2!->(aeval (l1 !-> aeval st a1;st) a2);(l1 !-> (aeval st a1);st)) =
            (l1!->(aeval (l2 !-> aeval st a2;st) a1);(l2 !-> (aeval st a2);st))).
    {
    apply aeval_weakening with (st := st) (ni := (aeval st a1)) in H0.
    apply aeval_weakening with (st := st) (ni := (aeval st a2)) in H1.
    rewrite H0. rewrite H1.
    apply functional_extensionality. intros.
    rewrite t_update_permute. + try reflexivity.  +try assumption. }
    rewrite H3.
...
Definition cequiv_for (c1 c2 : com) : Prop :=
  forall (st st' : state),
    (st =[ c1 ]=> st') <-> (st =[ c2 ]=> st').

Theorem for_while_equiv : forall c1 c2 c3 b,
  cequiv_for <{for c1 ; b ; c2 do c3}>
...
Proof.   
  unfold capprox; split; intro.
  - assert (empty_st =[c4]=> (X !-> 1)).
    { apply H. constructor. auto. }
    inversion H0; subst. simpl in H5.
    assert ((X !-> 2) X = (X !-> 1) X).
    { rewrite H5. reflexivity. }
    repeat rewrite t_update_eq in H1. discriminate.
...
Definition zprop (c : com) : Prop := exists st st',
st =[c]=> st'.
...
Proof.  
  unfold zprop; intros. destruct H as [st [st' H]].
 exists st, st'. unfold capprox in H0. apply H0. assumption.
Qed.
(* 2023-07-06 15:50 *)
