...
+ destruct (select i l) eqn:E.
  ...
  * apply perm_trans with ([a] ++ [n] ++ l0).
    ** simpl. 
       apply perm_skip. specialize IHl with i. rewrite E in IHl.
       apply IHl.
    ** apply Permutation_app_swap_app.
+ destruct (select a l) eqn:E.
  ...
  * simpl.
    apply perm_skip. specialize IHl with a. rewrite E in IHl.
    apply IHl.
  * apply Permutation_app_swap_app.
Qed.
...
intros. generalize select_perm. intros. specialize H1 with i al. rewrite H0 in H1.
apply Permutation_in with _ _ _ i in H1.
- apply in_inv in H1. destruct H1.
  + lia.
  + rewrite Forall_forall in H. specialize H with i. apply H. easy.
- apply in_eq.
Qed.
...
- bdestruct (i >=? a).
   * destruct (select i al) eqn:H1. inv H. apply Forall_cons.
      + apply select_biggest_aux with i al j l in H1.
          ++ lia.
          ++ apply IHal with i. easy.
      + apply IHal with i. easy.
   * destruct (select a al) as [j' l'] eqn : H'. inv H. apply Forall_cons.
      + apply select_biggest_aux with a al j l' in H'.
          ++ lia.
          ++ apply IHal with a. easy.
      + apply IHal with a. easy.
Qed.
...
Lemma delete_max_Some_relate:
  forall (p q: priqueue) k (pl ql: list key), priq p ->
   ...
   Permutation pl (k::ql) /\ Forall (ge k) ql.
Proof.
intros. inv H0. inv H2. induction pl.
- simpl in H1. inversion H1.
- simpl in H1. inversion H1. split.
  + generalize select_perm. intro. specialize H0 with a pl. rewrite H2 in H0. easy.
  + apply select_biggest with a pl. easy.
Qed.
