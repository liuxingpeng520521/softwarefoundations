...
Proof.
  ...
  +inversion Ht; subst. remember H2. clear Heqh. apply IHt1 in H2.
   apply IHt2 in H4. destruct H2 ,H4; right. 
   -eapply canonical_forms_fun in H . *destruct H. destruct H. exists (<{ [x0:=t2]x1 }>). rewrite H. auto.
                                      *apply h.
   -destruct H0. exists <{t1 x0}>. auto.
   -destruct H. exists <{x0 t2}>. auto.
   -destruct H. exists <{x0 t2}>. auto.
  +right. inversion Ht;subst. remember H3. clear Heqh. apply IHt1 in H3. destruct H3.
   -destruct (canonical_forms_bool t1); subst; eauto.
   -destruct H. exists <{ if x0 then t2 else t3 }>. auto.
Qed.
(** [] *)
...
Proof.
  exists <{if true then false else (x)}>. exists <{false}>. exists <{Bool}>.
  split. +constructor.
   +split. -constructor.
   -unfold not. intro. inversion H;subst. inversion H7; subst. inversion H2.
Qed.
...
Proof.
  ...
  apply IHhas_type1 in H5. inversion H5. reflexivity.
...
Proof.
  ...
  assert ( (y |-> T1; Gamma) x=Gamma x). {apply update_neq. assumption. }
  rewrite H3 in H2. exists x0. assumption.
Qed.
(** [] *)
...
Corollary typable_empty__closed : forall t T,
    ...
    closed t.
Proof.
  intros. unfold closed. intros x H1. eapply free_in_context in H.
  +destruct H;inversion H.
  +apply H1.
Qed.
(* 
  unfold closed. intros x H1. 
  assert (exists T':ty, empty x = Some T').
  { apply (free_in_context x t T empty). eauto. }
...
(* Theorem progress : forall t T, empty |- t \in T -> value t \/ exists t', t --> t'. *)
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
         0 any reducible term can reduce normally or reduce to zap.
      - Progress
         1 every term can reduce to zap.
      - Preservation
         1 any typed expression can reduce to zap, which can have any type
*)
...
(** **** Exercise: 2 stars, standard (stlc_variation2)
    ...
    false, give a counterexample.
      ...
      - Progress
         1
      - Preservation
         0 (\x:A, x) is well typed, but it's reduced form [foo]
        is not well typed.
*)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation4)
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
         0  (if true then false else true) now can reduce to both true and false.
      - Progress
         1
      - Preservation
         0 (if true then (\x:Bool, x) (\x: Bool, x))  has type (Bool -> Bool)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation5)
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
         1
      - Progress
         0 if ((\x, \y, x) true) then true else true is
              well typed but cannot reduce further nor is a value.
      - Preservation
         0  ((\x, \y, x) true) has type Bool, but it's reduction
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation7)
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
         1
      - Progress
         0 [if (\x:Bool, true) then true else true]
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_App2 : forall v1 t2 t2',
      ...
      t2 --> t2' ->
      <{v1 t2}> --> <{v1  t2'}>
  | ST_Succ1: forall t1 t2,
      t1-->t2->
      <{succ t1}>--><{succ t2}>
  | ST_Succ2: forall t n,
      t=tm_const n->
      <{succ t}>-->tm_const (S n)
  | ST_Pred1: forall t1 t2,
      t1-->t2->
      <{pred t1}>--><{pred t2}>
  | ST_Pred2: forall t n,
      t=tm_const n->
      <{pred t}>-->tm_const (pred n)
  ...
  | ST_Mult2 : forall v1 t2 t2',
      ...
      t2 --> t2' ->
      <{v1*t2}> --> <{v1*t2'}>
  | ST_Mult : forall v1 v2 n1 n2,
      v1 = tm_const n1 ->
      v2 = tm_const n2 ->
      <{v1*v2}> --> tm_const ((n1)*(n2))
  | ST_If : forall t1 t1' t2 t3,
      t1 --> t1' ->
      <{if0 t1 then t2 else t3}> --> <{if0 t1' then t2 else t3}>
  | ST_IfTrue : forall n t0 t1 t2,
      t0=tm_const (S n)->
      <{if0 t0 then t1 else t2}> --> t1
  | ST_IfFalse : forall t0 t1 t2,
      t0=tm_const 0->
      <{if0 t0 then t1 else t2}> --> t2
where "t '-->' t'" := (step t t').
...
Proof. 
  exists <{ 3 * 2 }>. eapply multi_step.
  +auto.
  +eapply multi_step. -constructor. auto.  -simpl. apply multi_refl.  Qed.
...
Proof. eapply T_App. +eapply T_App. -eapply T_Abs. eapply T_Abs. eapply T_Mult. *apply T_Var. auto.
                                                                                *auto.
                                    -eapply T_Const. reflexivity.
                     +eauto. Qed.
...
Lemma substitution_preserves_typing : forall Gamma x U t v T,
  ...
  Proof.
    ...
    induction t; intros T Gamma H;
    (* in each case, we'll want to get at the derivation of H *)
      inversion H; clear H; subst; simpl; eauto; try(inversion H0).
    - (* var *)
      rename s into y. destruct (eqb_spec x y); subst.
      + (* x=y *)
        rewrite update_eq in H2.
...
Proof with eauto. 
  ...
  induction HT;
    ...
    + (* ST_AppAbs *)
      apply substitution_preserves_typing with T2...
      inversion HT1...
  -inversion HE;subst... (* inversion HT;subst. eapply T_Const. reflexivity.*)
  -inversion HE;subst... (*inversion HT;subst. eapply T_Const. reflexivity.*)
  -inversion HE;subst...
Qed.
(** [] *)
...
Lemma canonical_forms_sp : forall t,
  empty |-- t \in Nat ->
  value t ->
...
Proof.
  intros t HT HVal.
  destruct HVal;subst.
  +inversion HT; subst. exists n. assumption. 
  +inversion HT;subst. exists n0. assumption.
Qed.
...
Theorem progress : forall t T,
  ...
  (*T_Succ*)
  -right. destruct IHHt...
   *eapply canonical_forms_sp in H. 
    +destruct H.  exists (S x0). auto.
    +assumption.
   *destruct H. exists <{succ x0}>. auto.
  (*T_Pred*)
  -right. destruct IHHt...
    *eapply canonical_forms_sp in H. 
     +destruct H.  exists (pred x0). auto.
     +assumption.
    *destruct H. exists <{pred x0}>. auto.
  (* T_Mult *)
  -right. destruct IHHt1...
   +destruct IHHt2...
    *eapply canonical_forms_sp in H; eapply canonical_forms_sp in H0.
     ++destruct H ,H0. exists (x0*x1). auto. 
     ++assumption.
     ++assumption.
     ++assumption.
    *destruct H0. exists <{t1 * x0}>. auto.
  ...
  - (* T_If *)
    ...
    + (* t1 is a value *)
      ...
      *destruct H. destruct x0.
       ++exists <{t3}>. auto.
       ++exists <{t2}>. apply ST_IfTrue with x0. auto.
      *assumption.
    + (* t1 also steps *)
