...
Proof. 
  apply T_App with <{{i1:(A->A)::i2:(B->B)::nil}}>. 
  +apply T_Abs. -apply wfRCons. *apply wfArrow. ++apply wfBase. ++apply wfBase.           
                                *apply wfRCons. ++apply wfArrow. --apply wfBase. --apply wfBase. ++apply wfRNil. ++apply RTnil. 
                                *apply RTcons.
                -apply T_Proj with <{{"i1" : A -> A::  "i2" : B -> B:: nil }}>.
                                *apply T_Var. ++rewrite update_eq. reflexivity.
                                              ++apply wfRCons. +++apply wfArrow. --apply wfBase. --apply wfBase. +++apply wfRCons. ++++apply wfArrow. --apply wfBase. --apply wfBase. ++++apply wfRNil. ++++apply RTnil. 
                                                               +++apply RTcons.
                                *simpl. reflexivity.
  ...
  ~ exists T,
    (y |-> A) |--
     (\a : ( i1 : A  :: nil ), a --> i1 )
      ( i1 := y :: i2 := y :: nil )  \in T.
Proof.
  intros. intro. destruct H. inversion H;subst. solve_by_inverts 3. Qed. 
(** [] *)
