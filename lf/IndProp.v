...
Proof.
  intros. induction H as [|n' H' IE].
  -simpl. apply H0.
  -simpl. apply ev_SS. apply IE.
Qed.
(** [] *)
...
Proof.
  intros. induction H0.
  -simpl in H. apply H.
  -apply IHev. simpl in H. apply evSS_ev in H. apply H.
Qed.
...
Proof.
  ...
  +assert (E: n + m + (n + p) = (n + n) + (m + p)).
  { rewrite add_assoc.  rewrite add_assoc. 
    assert (H1: n+m+n=n+n+m).
     { rewrite <- add_assoc. 
       rewrite <- add_assoc. 
       assert (eq: m+n=n+m). { apply add_comm. }
       rewrite eq. reflexivity.  
     }
...
Inductive total_relation : nat -> nat -> Prop :=
  |tr_zero (n:nat) : total_relation 0 n
  |tr_succ (n m:nat) (H:total_relation m n) :total_relation (S m) n.
...
Theorem empty_relation_is_empty : forall n m, ~ empty_relation n m.
  Proof.
  intros. unfold not. intro. inversion H. Qed. 
(** [] *)
...
Proof.
  ...
  +induction m.
   -right. unfold ">=". apply O_le_n.
   -destruct IHn with m.
    *left. unfold lt in *. apply n_le_m__Sn_le_Sm. apply H. 
    *right. unfold ">=" in *. apply n_le_m__Sn_le_Sm. apply H.
Qed. 
...
Proof.
  intros.
  split.
  - rewrite <- le_plus_l in H. apply H.
  - rewrite add_comm in H. rewrite <- le_plus_l in H. apply H.
Qed.
...
Proof.
  ...
  +intros. induction p.
   -right. apply plus_le in H as [H H2]. simpl in H2. apply H2.
...
Theorem plus_le_compat_r : forall n m p,
  ...
  n + p <= m + p.
Proof.
  intros. rewrite add_comm. rewrite add_comm with m p.
  apply plus_le_compat_l. apply H.
Qed.

Theorem Sn_trans_n:forall n m, S n<=m->n<=m.
Proof.
  intros. generalize dependent n. induction m.
  +intros. inversion H.
  +intros. induction n.
   -apply O_le_n.
   -apply Sn_le_Sm__n_le_m in H. apply IHm in H.
...
Theorem le_plus_trans : forall n m p,
  ...
  n <= m + p.
Proof.
  intros. rewrite add_comm. generalize dependent p. induction n.
  +intros. apply O_le_n.
  +intros. induction p.
   *simpl. apply H.
   *simpl. apply n_le_m__Sn_le_Sm. apply IHn. apply Sn_trans_n. apply H.
Qed.
...
Theorem n_lt_m__n_le_m : forall n m,
  ...
  n <= m.
Proof.
intros. apply Sn_trans_n. unfold lt in H. apply H. Qed.
...
Proof.
  intros n1 n2 m H.
  assert (H': S n1 <= S (n1 + n2) /\ S n2 <= S (n1 + n2)).
  split.
  - apply n_le_m__Sn_le_Sm. apply le_plus_l.
  - apply n_le_m__Sn_le_Sm. rewrite add_comm. apply le_plus_l.
  - destruct H' as [H1 H2]. split.
    + apply le_trans with (n:= S (n1+n2)). apply H1. apply H.
    + apply le_trans with (n:= S (n1+n2)). apply H2. apply H.
Qed.
...
Proof.
  ...
  - intro. apply O_le_n.
  - intro E. inversion E.
  - intro E. apply n_le_m__Sn_le_Sm. apply IHn.
    simpl in E. apply E.
Qed.
...
Proof.
  ...
  - intros n E. inversion E. reflexivity.
  - intros n E. destruct n as [| n'].
    + reflexivity.
    + simpl. apply IHm'. apply Sn_le_Sm__n_le_m. apply E.
Qed.
...
Proof.
  intros.
  apply leb_correct. apply leb_complete in H. apply leb_complete in H0.
...
(** - Which of the following propositions are provable?
    ...
    - If we dropped constructor [c5] from the definition of [R],
      ...
      sentence) explain your answer. 涓嶄細
...
Theorem R_equiv_fR1 : forall n, R 0 n n.
...
Proof.
  ...
  +intro. induction H.  
   ...
   -unfold fR in IHR. simpl in IHR. rewrite<-plus_n_Sm in IHR. injection IHR. unfold fR. intro. apply H0.
   -unfold fR. unfold fR in IHR. rewrite <- IHR. apply add_comm.
  +intro. generalize dependent o.  induction m ,n.
   -intros. inversion H. simpl. apply c1.
   -intros o E. simpl in E. inversion E. apply c3. apply R_equiv_fR1.
   -intros o E. unfold fR in E. rewrite add_comm in E. simpl in E.
    rewrite <- E. apply c2. apply IHm. unfold fR. rewrite add_comm. reflexivity.
   -intros o E. unfold fR in E. simpl in E. rewrite <- E. apply c2. apply IHm. unfold fR. reflexivity.
Qed.
...
(** There are several equivalent ways of writing inductive
    types. 聽We've mostly seen this style... *)
...
Proof.
  ...
  - simpl. intros. rewrite-> app_length in H. apply add_le_cases in H.
    ...
    +apply IH2 in H2. destruct H2 as [x  [x0  [x1  H]]]. destruct H as [H1 [H2 H3]].
     exists (s1 ++ x). exists x0. exists x1. split.
     *rewrite <- app_assoc. rewrite H1. reflexivity.
     *split.
      ++apply H2.
      ++intros m. rewrite <- app_assoc. apply MApp. apply Hmatch1. apply H3.
  (* MUninoL *)
  - simpl. intros H. apply plus_le in H. destruct H as [H1 H2]. apply IH in H1.
    destruct H1 as [x  [x0  [x1  H]]]. destruct H as [H3 [H4 H5]]. 
  ...
  - simpl. intros H. apply plus_le in H. destruct H as [H1 H2]. apply IH in H2.
    destruct H2 as [x  [x0  [x1  H]]]. destruct H as [H3 [H4 H5]]. 
    exists x. exists x0. exists x1. split.
    + apply H3.
    + split.
     * apply H4. 
  ...
  - simpl. destruct s1.
   +simpl. apply IH2.
   +intros. remember (x::s1) as s. 
    destruct (pumping_constant re <=? length s) eqn:E.
    *apply leb_iff in E. apply IH1 in E. destruct E as [x'  [x0  [x1  E]]]. destruct E as [H1 [H2 H3]].
      exists x'. exists x0. exists (x1 ++ s2). split.
      **rewrite -> app_assoc. rewrite -> app_assoc. rewrite <- (app_assoc T x' x0 x1).
      rewrite <- H1. reflexivity.
      **split.
      ++apply H2.
      ++intros m. rewrite -> app_assoc. rewrite -> app_assoc. rewrite <- (app_assoc T x' (napp m x0) x1). apply MStarApp. apply H3. apply Hmatch2.
    *apply le_false_rev in E.  exists nil, s, s2. simpl. split.
     ** reflexivity.
     ** split.
...
Proof.
  ...
  - simpl. intros H. rewrite -> app_length in H. apply add_le_cases in H.
    destruct H as [H1 | H2].
  (* MApp-1 pumping_constant re1 <= length s1 *)
  ...
  (* MApp-2 pumping_constant re2 <= length s2 *)
  -- destruct (pumping_constant re1 <=? length s1) eqn:E.
     (* 鍜孧Add-1瀹屽叏涓€鑷? pumping_constant re1 <= length s1 *)
     * apply leb_iff in E. apply IH1 in E.  destruct E as (x & x0 & x1 & H). destruct H as (H3 & H4 & H5 & H6).
     ...
     * apply le_false_rev in E. apply IH2 in H2. destruct H2 as (x & x0 & x1 & H). destruct H as (H1 & H2 & H3 & H5).
       exists (s1 ++ x), x0, x1. split;[|split ;[|split]].
     ** rewrite <- app_assoc. rewrite <- H1. reflexivity.
     ** apply H2.
     ** rewrite -> app_length. rewrite <-add_assoc. apply le_plus_both.
        apply E. apply H3.
  ...
  -- intros H. remember (x::s1) as s. simpl in H.
     destruct (pumping_constant re <=? length s) eqn:E.
     * apply leb_iff in E. apply IH1 in E. destruct E as (x0 & x1 & x2 & Ha & Hb & Hc & Hd).
     ...
     * apply le_false_rev in E. exists nil, s, s2. simpl. split. reflexivity. split.
     ** unfold not. intros Hfalse. rewrite -> Heqs in Hfalse. discriminate.
     ** split. apply E. intros m. apply napp_star. apply Hmatch1. apply Hmatch2.
Qed.
...
Proof.
  ...
  - simpl in H1, H2. destruct (eqbP n x) as [E | E].
    +discriminate H1.
    +inversion H2. 
     *rewrite H in E. unfold not in E. apply E. reflexivity.
     *simpl in H1. apply IH. apply H1. apply H.
Qed.
...
| ns_cons x x' l (H0:x <> x') (H:nostutter (x' :: l)): nostutter (x :: x' :: l).
...
Proof. apply ns_cons. discriminate. apply ns_cons. discriminate. apply ns_cons. discriminate.
apply ns_cons. discriminate. apply ns_cons. discriminate. apply ns_one. Qed.
(* 
...
Proof. unfold not. intros. inversion H. inversion H4.  destruct H7. reflexivity. Qed.
(* 
  Proof. intro.
...
Inductive merge {X: Type} : list X -> list X -> list X -> Prop :=
    | merge_nil : merge [] [] []
    | merge_left  x xs ys zs (H:merge xs ys zs) :
                  merge (x :: xs) ys (x :: zs)
    | merge_right y xs ys zs (H:merge xs ys zs) :
                    merge xs (y :: ys) (y :: zs).
...
Proof.
  ...
  +simpl in *. destruct H0.
   rewrite H0. assert (A:filter test zs = xs -> x :: filter test zs = x :: xs ).
   { intro. rewrite->H3. reflexivity. }
   apply A. apply IHmerge. 
    *apply H2.
    *apply H1.
  +simpl in *. destruct H1.
  rewrite H1. apply IHmerge. 
   *apply H0.
   *apply H2.
Qed.
...
Proof.
  intros. induction l.
  +simpl. apply c0.
  +simpl. rewrite app_assoc. apply c2 with (l:=l++rev l). apply IHl. Qed.
...
(*Inductive ListConsSnocView {X : Type} : list X -> Type :=
  ...
  | cs_single : forall x : X, ListConsSnocView [x]
  | cs_cc : forall (x y : X) {xs : list X},
    ListConsSnocView xs -> ListConsSnocView (x :: xs ++ [y])
.*)
Inductive ListConsSnocView {X : Type} : list X -> Type :=
  | cs_nil : ListConsSnocView []
  | cs_single x: ListConsSnocView [x]
  | cs_cc  x y  {xs : list X} (view:ListConsSnocView xs) : ListConsSnocView (x :: xs ++ [y])
.
...
Theorem list_cons_snoc_view_tr: forall {X : Type}
...
- set (view' := append_snoc view h).
  ...
  assumption.
Qed.
Lemma app_inv_tail:(*coq标准库引理*)
    forall {X:Type} (l l1 l2 : list X), l1 ++ l = l2 ++ l -> l1 = l2. Admitted.
Definition list_cons_snoc_view {X : Type} (xs : list X) : ListConsSnocView xs :=
    list_cons_snoc_view_tr xs cs_nil.
(*Theorem list_cons_snoc_view' : forall {X : Type} (xs : list X) ,ListConsSnocView xs.
Proof. intros.
  assert(nil++xs=xs). {reflexivity. } rewrite<-H. apply list_cons_snoc_view_tr.
  apply cs_nil. Qed.*)
Theorem palindrome_converse : forall {X : Type} (l : list X), 
    l = rev l -> pal l.
Proof. intros.
(*induction (list_cons_snoc_view' l) as [| x | x y l' view' IH].*)
induction (list_cons_snoc_view l) as [| x | x y l' view' IH].
+apply c0.
+apply c1.
+simpl in H. rewrite rev_app_distr in H. simpl in H.
inversion H;subst. apply c2. apply IH. 
apply app_inv_tail in H2. 
apply H2.
Qed.

(*second way*)
Lemma rev_pal_length : forall X (n : nat) (l : list X),
  length l <= n -> l = rev l -> pal l.
Proof.
  intros. generalize dependent l. induction n.
  - intros. destruct l. apply c0. inversion H.
  - destruct l.
    + intros. apply c0.
    + intros. simpl in H. apply Sn_le_Sm__n_le_m in H. simpl in H0. destruct (rev l) eqn : E.
      * destruct l. apply c1. simpl in E. destruct (rev l). discriminate E. discriminate E.
      * injection H0. intros. destruct H2. rewrite H0. apply c2. apply IHn.
        -- apply le_trans with (length l). rewrite H1. rewrite app_length. apply le_plus_l. apply H.
        -- rewrite H1 in E. rewrite rev_app_distr in E. simpl in E. injection E. intros. symmetry. apply H2.
Qed.

Theorem palindrome_converse': forall {X: Type} (l: list X),
    l = rev l -> pal l.
Proof.
  intros. apply rev_pal_length with (n := length l). apply le_n. apply H.
Qed.
...
|jointnil l :disjoint nil l
|joint1 l1 l2 a (H: disjoint l1 l2) (H0: ~(In a l2)) : disjoint (a::l1) l2.
...
|NODupnil :NoDup nil
|Dup1 l a (H: NoDup l) (H0: ~(In a l)) : NoDup (a::l).

(** Finally, state and prove one or more interesting theorems relating
    [disjoint], [NoDup] and [++] (list append).  *)
Search In.
Theorem mytry: forall {X:Type} (l1 l2 :list X), NoDup l1 ->NoDup l2 -> disjoint l1 l2 -> NoDup (l1++l2).
Proof. intros.
  induction H1.
  +simpl. apply H0.
  +simpl. apply Dup1.
   -apply IHdisjoint. inversion H. apply H5. apply H0.
   -inversion H. 
    intro. apply In_app_iff in H7. destruct H7.
     ++apply H6. apply H7.
     ++apply H2. apply H7.
Qed.
(* Do not modify the following line: *)
...
Proof.
  ...
  +inversion H.
  +inversion H.
   -rewrite H0. exists nil. exists l. reflexivity.
   -apply IHl in H0. destruct H0 as [l1 [l2 H0]]. rewrite H0.
   exists (x0 :: l1). exists l2. simpl. reflexivity.
Qed.
        ...
        * (* ~ In x l1' *) 
          ...
          -- (* Eyl2: y :: l2' = la ++ x :: lb *)
             ...
             ++ (* must prove [length l2 < length l1'] where [l2 = la ++ lb] *)
                ...
                apply Sn_le_Sm__n_le_m in Hlen. unfold lt. rewrite app_length. apply Hlen.
  Qed.
...
(** **** Exercise: 3 stars, standard, optional (app_ne)
    ...
    use it later. *)
Lemma cons_app_asso: forall {X:Type} (a:X) l1 l2, a::l1++l2=(a::l1)++l2.
...
Proof.
  ...
  +intros. inversion H.
   induction s1.
    -simpl. left. split. apply H3. apply H4.
    -simpl in *. right. inversion H1. exists s1 ,s2. split. reflexivity.  split. rewrite<-H6. apply H3. apply H4. 
  +intros. destruct H.
   -destruct H. assert(a::s=[]++(a::s)). {reflexivity. } rewrite H1. apply MApp. apply H. apply H0.
   -destruct H as [s0 [s1 [H [H0 H1]]]].  rewrite H. rewrite cons_app_asso. apply MApp. apply H0. apply H1.
Qed.
...
Proof.
  intros. split.
  +intros. remember(a::s) as s0. remember (Star re) as re0. induction H.
   -discriminate.
   -discriminate.
   -discriminate.
   -discriminate.
   ...
   -discriminate.
   -induction s1.
    *simpl in *. apply IHexp_match2 in Heqs0.
     ++destruct Heqs0 as [s0 [s3 H1]].
       exists s0,s3. apply H1.
     ++apply Heqre0.
    *inversion Heqs0. exists s1,s2. split. reflexivity. split.
     ++inversion Heqre0. rewrite<-H4. rewrite<-H2. apply H.
     ++apply H0.
  +intros. destruct H as [s0 [s1 [H0 [H1 H2]]]].
   rewrite H0. rewrite cons_app_asso. apply MStarApp. apply H1. apply H2.
Qed.
(** [] *)
...
Proof.
  ...
  +intros. remember (a :: s) as s0.
   induction H.
   -discriminate.
   -inversion Heqs0. simpl. assert( eqb a a = Datatypes.true). {apply eqb_eq. reflexivity. }
    rewrite H. apply MEmpty.
   -simpl. destruct( match_eps_refl re1).
    *induction s1.
     ++apply MUnionR. apply IHexp_match2. simpl in Heqs0. apply Heqs0.
     ++inversion Heqs0. apply MUnionL. apply MApp.
      --induction s2.
        **rewrite appnil in *. rewrite H4. apply IHexp_match1. rewrite H3,H4. reflexivity.
        ***)
  unfold derives. unfold is_der. split.  
  +generalize dependent s. induction re.
   -intros. inversion H.
   -intros. inversion H.
   -intros. simpl. inversion H. assert( eqb t t = Datatypes.true). {apply eqb_eq. reflexivity. } 
    rewrite H0. apply MEmpty.
   -intros. inversion H. destruct( match_eps re1) eqn:E.
    *destruct s1 as [| h1 t1].
      ++simpl. rewrite E. apply MUnionR. apply IHre2. simpl in H1. rewrite H1 in H4. apply H4.
      ++simpl. rewrite E. apply MUnionL. inversion H1. apply MApp.
      { apply IHre1. rewrite <- H6. assumption. }
      { assumption. }
    *destruct s1 as [| h1 t1].
      ++simpl in H1. destruct (match_eps_refl re1).
      discriminate E. contradiction.
      ++ inversion H1. simpl. rewrite E. apply MApp.
      { apply IHre1. rewrite <- H6. assumption. }
      { assumption. }
    - intros. inversion H.
     * simpl. apply MUnionL. apply IHre1. assumption.
     * simpl. apply MUnionR. apply IHre2. assumption.
    -intros. simpl. remember (a :: s) as sa. remember (Star re) as re'.
      induction H; try discriminate Heqsa; try discriminate Heqre'.
      destruct s1 as [| h1 t1]; simpl in Heqsa.
      * apply IHexp_match2; assumption.
      * inversion Heqsa. apply MApp.
        { apply IHre.
          rewrite <- H2. inversion Heqre'. rewrite <- H4. assumption. }
        { assumption. }
  + generalize dependent s. induction re; simpl; intros.
   -inversion H.
   -inversion H.
   -destruct ( eqb t a ) eqn:E; inversion H. apply eqb_eq in E. rewrite E. apply MChar.
   -destruct (match_eps re1) eqn: E.
    *inversion H;subst.
     ++inversion H2;subst. rewrite cons_app_asso. apply MApp. apply IHre1. apply H4. apply H5.
     ++rewrite<-app_nil_l with (a::s). apply MApp.
      --destruct (match_eps_refl re1). assumption. discriminate E.
      --apply IHre2. assumption.
    *inversion H;subst. rewrite cons_app_asso. apply MApp.
    { apply IHre1. assumption. }
    { assumption. }
   -inversion H.
    * apply MUnionL. apply IHre1. assumption.
    * apply MUnionR. apply IHre2. assumption.
   - inversion H. rewrite cons_app_asso. apply MStarApp. apply IHre. assumption. assumption.
Qed.
