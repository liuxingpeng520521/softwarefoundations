...
Proof.
 induction T;  intros t t' E Rt; unfold R; fold R; unfold R in Rt; fold R in Rt;
  ...
  +destruct RRt as [s1 [s2 [H0 [H1 H2]]]].
   inversion H0;subst.
   -inversion E;subst;split; try(eapply preservation);eauto;
...
Proof with eauto.
  induction T; intros t t' Ha E Rt; unfold R; fold R; unfold R in Rt; fold R in Rt;
  destruct Rt as [typable_empty_t [halts_t RRt]];split...
  +split... eapply step_preserves_halting...
  +split. try(eapply step_preserves_halting)... intros.
...
Proof with eauto.
  ...
  contradiction H...
   - unfold subst. rewrite E...
  +simpl. destruct (String.eqb x s) eqn :E... rewrite String.eqb_neq in E.
...
Proof with eauto.
 induction t; intros; simpl...
  ...
  destruct (eqb_spec x s); destruct (eqb_spec x1 s)...
   +subst. exfalso...
   +subst. simpl. rewrite String.eqb_refl. destruct (eqb_spec x1 s)... exfalso...
   +subst. simpl. rewrite String.eqb_refl. destruct (eqb_spec x s)... exfalso...
   + simpl. rewrite false_eqb_string... rewrite false_eqb_string...
...
Lemma msubst_pair : forall ss t1 t2, msubst ss <{(t1, t2)}>
...
Proof.
  ...
  assert (empty |-- <{if {msubst env0 t1} then {msubst env0 t2} else {msubst env0 t3}}> \in T1).
  ...
  assert (<{{msubst env0 t0}.fst}> -->* v1).
  apply multi_trans with <{(v1 ,v2).fst}>; eauto.
  apply multistep_Fst.
  apply multi_trans with <{(s1,s2)}>; auto.
  apply multi_trans with <{(v1, s2)}>.
  ...
  destruct (R_halts R_s1) as [v1 [H11 H12]].
  destruct (R_halts R_s2) as [v2 [H21 H22]].
  assert (<{{msubst env0 t0}.snd}> -->* v2).
  apply multi_trans with <{(v1 ,v2).snd}>; eauto.
  apply multistep_Snd.
  apply multi_trans with <{(s1,s2)}>; auto.
  apply multi_trans with <{(v1, s2)}>.
  apply multistep_Pair1; auto.
