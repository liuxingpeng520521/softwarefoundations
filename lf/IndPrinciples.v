...
Proof. unfold booltree_ind_type. intros.
apply booltree_ind.
+apply H.
+apply H0.
...
Theorem Toy_correct : exists f g,
  forall P : Toy -> Prop,
    ...
    forall t : Toy, P t.
Proof. exists con1, con2. apply Toy_ind. Qed.
...
Definition better_t_tree_ind_type : Prop:=
  ...
  (P t_leaf) -> 
  (forall (tl: t_tree X) (v : X) (tr : t_tree X), P tl -> P tr -> P (t_branch (tl, v, tr))) ->
...
Definition better_t_tree_ind : better_t_tree_ind_type:=
  fun X P=>fun P0=>fun P1=>
    fix f (t:t_tree X):=
     match t with
     |t_leaf=>P0
     |t_branch (tl, v, tr) => P1 tl v tr (f tl) (f tr)
     end.
...
Theorem reflect_involution : forall (X : Type) (t : t_tree X),
    reflect (reflect t) = t.
Proof.   intros X. apply better_t_tree_ind.
- unfold reflect. reflexivity.
- intros tl v tr Htl Htr. 
  simpl. rewrite Htl. rewrite Htr. reflexivity.
Qed.
