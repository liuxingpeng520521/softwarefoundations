...
Proof.
  ...
  generalize c.  
  induction p; intros.
  - destruct c0, q; simpl;try(rewrite IHp; lia).
    + rewrite succ_correct. lia.
    + rewrite succ_correct. lia.
  - destruct c0, q; simpl;try(rewrite IHp; lia).
    + rewrite succ_correct. lia.
    +lia.
  - destruct c0, q; simpl; try(rewrite succ_correct; lia);try(reflexivity).
  Qed.
...
Proof.
  ...
  + specialize (IHx y). destruct (compare x y) eqn:H; lia.
  + specialize (IHx y). destruct (compare x y) eqn:H; lia.
  + pose (positive2nat_pos x). lia.
  + specialize (IHx y). destruct (compare x y) eqn:H; lia.
  + specialize (IHx y). destruct (compare x y) eqn:H; lia.
  + pose (positive2nat_pos x). lia.
  + pose (positive2nat_pos y); lia.
  + pose (positive2nat_pos y); lia.
  +lia.
Qed.
