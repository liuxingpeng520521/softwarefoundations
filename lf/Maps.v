...
Theorem t_update_neq : forall (A : Type) (m : total_map A) x1 x2 v,
  ...
  (x1 !-> v ; m) x2 = m x2.
Proof.
  intros. unfold t_update. rewrite<-String.eqb_neq in H. rewrite H. reflexivity.
Qed.
(** [] *)
...
Lemma t_update_shadow : forall (A : Type) (m : total_map A) x v1 v2,
  (x !-> v2 ; x !-> v1 ; m) = (x !-> v2 ; m).
Proof.
  intros. unfold t_update. apply functional_extensionality. intro. destruct((x =? x0)%string) eqn:E.
  +reflexivity.
  +reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  +rewrite String.eqb_eq in E. rewrite E. reflexivity.
  +reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  +destruct((x2 =? x)%string) eqn:F.
   -rewrite String.eqb_eq in *. rewrite E in H. rewrite F in H. destruct H. reflexivity.
   -reflexivity.
  +destruct((x2 =? x)%string) eqn:F.
   -reflexivity.
   -reflexivity.
