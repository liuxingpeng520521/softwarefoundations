...
Proof.
  intros.
  induction b; try (simpl; reflexivity); try (simpl; rewrite optimize_0plus_sound; rewrite optimize_0plus_sound; reflexivity).
  +simpl. induction b; try (simpl; reflexivity); try (simpl; rewrite optimize_0plus_sound; rewrite optimize_0plus_sound; reflexivity); try (rewrite IHb; reflexivity).
...
Proof.
  split.
  - (* -> *)
    intros H; induction H; try(subst; reflexivity);
    try(simpl; rewrite aeval_iff_aevalR in H; rewrite aeval_iff_aevalR in H0; rewrite H0,H; reflexivity).
  - (* <- *)
    intros H. generalize dependent bv. induction b; intros; simpl in H; subst; constructor; try (rewrite aeval_iff_aevalR; reflexivity); try (apply IHb; reflexivity).
...
Proof.
  ...
  (** Proceed by induction on the assumed derivation showing that
      ...
      +injection Heqloopdef. intros. rewrite H1 in H. simpl in H. discriminate H.
      +injection Heqloopdef. intros. apply IHcontra2. rewrite H0. rewrite H1. reflexivity.
    Qed. 
(** [] *)
...
|NW_Seq (c1 c2:com): 
       ...
       no_whilesR c2->
       no_whilesR <{ c1 ; c2 }>
|NW_If (test:bexp) (c1 c2:com):
       no_whilesR c1->
       no_whilesR c2->
       no_whilesR  <{ if test then c1 else c2 end }>.
Theorem no_whiles_eqv:
  forall c, no_whiles c = true <-> no_whilesR c.
Proof.
  intro. split.
  +intro. induction c; try constructor; try (simpl in H; apply andb_true_iff in H; destruct H);
  try (apply IHc1; apply H); try (apply IHc2; apply H0). discriminate H.
  +intro. induction H; try (simpl; reflexivity).
...
Lemma no_whiles_terminating: forall c st,no_whilesR c->exists st', st=[c]=>st'.
intros. generalize dependent st. induction H.
+intros. exists st. apply E_Skip.
+intros. exists (X0!->aeval st a;st). apply E_Asgn. reflexivity.
+intros. destruct IHno_whilesR1 with st. destruct IHno_whilesR2 with x. exists x0. apply E_Seq with x.
 -apply H1. -apply H2.
+intros. destruct IHno_whilesR1 with st. destruct IHno_whilesR2 with st. destruct(beval st test) eqn:E.
 -exists x. apply E_IfTrue. *apply E. *apply H1.
 -exists x0. apply E_IfFalse. *apply E. *apply H2.
Qed.
(* Do not modify the following line: *)
...
Proof.
  intros st e. induction e; simpl; intros; try (simpl; reflexivity).
  +rewrite execute_app. rewrite execute_app. rewrite IHe1. rewrite IHe2. simpl. reflexivity.
  +rewrite execute_app. rewrite execute_app. rewrite IHe1. rewrite IHe2. simpl. reflexivity.
  +rewrite execute_app. rewrite execute_app. rewrite IHe1. rewrite IHe2. simpl. reflexivity.
Qed.
...
Proof.
  intros. apply s_compile_correct_aux with (stack:=[]).
Qed.
...
Lemma beval_iff:forall st b, beval st b=beval' st b.
Proof. intros. destruct b;simpl;reflexivity. Qed.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_Seq1 : forall c1 c2 st st' st''  r2,
      st  =[ c1 ]=> st' / SContinue  ->
      st' =[ c2 ]=> st'' / r2 ->
      st  =[ c1 ; c2 ]=> st'' / r2
  | E_Seq2 : forall c1 c2 st st',
      st  =[ c1 ]=> st' / SBreak ->
      st  =[ c1 ; c2 ]=> st' / SBreak
  | E_IfTrue : forall st st' b c1 c2 r1,
      beval st b = true ->
      st =[ c1 ]=> st'  / r1->
      st =[ if b then c1 else c2 end]=> st' / r1
  | E_IfFalse : forall st st' b c1 c2 r1,
      beval st b = false ->
      st =[ c2 ]=> st' / r1 ->
      st =[ if b then c1 else c2 end]=> st' / r1
  | E_WhileFalse : forall b st c,
...
Proof.
  intro. induction c.
  +intros. inversion H; inversion H0;subst. split;reflexivity.
  +intros. inversion H; inversion H0;subst. split;reflexivity.
  +intros. inversion H; inversion H0;subst. split;reflexivity.
  +intros. inversion H; inversion H0;subst.
   -destruct (IHc1 _ _ _ _ _ H3 H10). subst. destruct (IHc2 _ _ _ _ _ H7 H14). subst. split;reflexivity.
   -destruct (IHc1 _ _ _ _ _ H3 H13). inversion H2.
   -destruct (IHc1 _ _ _ _ _ H6 H9). inversion H2.
   -destruct (IHc1 _ _ _ _ _ H6 H12). subst. split;reflexivity.
  +intros. inversion H; inversion H0;subst.
   -destruct (IHc1 _ _ _ _ _ H8 H16). subst. split;reflexivity.
   -rewrite H7 in H15. inversion H15.
   -rewrite H7 in H15. inversion H15.
   -destruct (IHc2 _ _ _ _ _ H8 H16). subst. split;reflexivity.
  +intros. inversion H; inversion H0;subst;try(rewrite H6 in H9; inversion H9);
  try(rewrite H3 in H14; inversion H14);try(rewrite H3 in H13; inversion H13);try(inversion H8);try(split;reflexivity).
  -destruct (IHc _ _ _ _ _ H7 H11). inversion H2.
  -destruct (IHc _ _ _ _ _ H7 H14). rewrite H1. split;reflexivity.
Qed.
...
Notation "'for' a ; b ; c 'do' d " :=
             (CFor a b c d) 
                (in custom com at level 89, a at level 79, b at level 79, c at level 79, d at level 79) : com_scope.
...
| E_WhileTrue : forall st st' st'' b c,
          ...
          st' =[ while b do c end ]=> st'' ->
          st  =[ while b do c end ]=> st''
| E_For: forall st st' a b c d,
          st  =[ a; while b do d ; c end ]=> st'->
          st=[ for a ; b ; c do d ]=>st'
      where "st =[ c ]=> st'" := (ceval c st st').    
