...
Example some_term_is_stuck :
  exists t, stuck t.
Proof.
  unfold stuck. exists <{ succ true }>. split.
  +unfold normal_form. unfold not. intros. inversion H. inversion H0. inversion H2.
  +unfold not. intros. inversion H; inversion H0.  inversion H2.
Qed.
...
Proof.
  unfold step_normal_form. unfold not. intros.  destruct H0. generalize dependent x. induction t; intros.
  +inversion H0. 
  +inversion H0.
  +inversion H; inversion H1. 
  +inversion H0.
  +inversion H; subst; inversion H1; subst. inversion H0; subst. eauto.
  +inversion H; inversion H1.
  +inversion H; inversion H1.
Qed. 
...
Proof with eauto.
   ...
   +inversion H0; subst. -reflexivity. -inversion H4.
   +inversion H0; subst. -inversion H. -inversion H. -apply IHstep in H5. rewrite H5. reflexivity.
   +inversion H0; subst. apply IHstep in H2. rewrite H2. reflexivity.
   +inversion H0; subst. -reflexivity. -inversion H1.
   +inversion H0; subst. -reflexivity. 
       -assert (value <{succ v}>) by auto.  apply value_is_nf in H1. unfold step_normal_form in *. exfalso. eauto.
   +inversion H0;subst. -inversion H. 
       -assert (value <{succ y2}>) by auto.  apply value_is_nf in H1. unfold step_normal_form in *. exfalso. eauto.
       -apply IHstep in H2. rewrite H2. reflexivity. 
   +inversion H0;subst. -reflexivity. -inversion H1.
   +inversion H0;subst. -reflexivity. -assert (value <{succ v}>) by auto.  apply value_is_nf in H1. unfold step_normal_form in *. exfalso. eauto.
   +inversion H0;subst. -inversion H. -assert (value <{succ v}>) by auto.  apply value_is_nf in H1. unfold step_normal_form in *. exfalso. eauto.
                        -apply IHstep in H2. rewrite H2. reflexivity.
Qed.
...
Example succ_hastype_nat__hastype_nat : forall t,
  ...
  |-- t \in Nat.
Proof.
  intros. inversion H; subst. apply H1. Qed.
(** [] *)
...
Proof.
  ...
  -destruct IHHT.
    +left. apply (nat_canonical t1 HT) in H.
     auto.
    +right. destruct H. exists (<{succ x}>). auto.
  -destruct IHHT.
    +right. apply (nat_canonical t1 HT) in H. inversion H.
   *exists <{0}>. constructor.
   *exists <{t}>. auto.
    +right. destruct H. exists (<{pred x}>). auto.
  -destruct IHHT.
    +right.  apply (nat_canonical t1 HT) in H. inversion H.
     *exists <{true}>. auto.
     *exists <{false}>. auto.
    +right. destruct H. exists <{iszero x}>. auto.
Qed.
...
Proof.
  ...
  induction HT;
    ...
    -inversion HE; subst. apply IHHT in H0. apply T_Succ. assumption.
    -inversion HE; subst. 
     +assumption.
    ...
    -inversion HE; subst.
     +constructor.
     +constructor.
     +apply IHHT in H0. apply T_Iszero. assumption.
Qed.
    (** [] *)
...
generalize dependent T.
induction H0; intros. 
  - (* T_If *) inversion H; subst. assumption. 
  - (* T_If *) inversion H; subst. assumption. 
  -inversion H; subst. apply T_If. +apply IHstep in H4. assumption. +assumption. +assumption.
  -inversion H; subst. apply T_Succ. apply IHstep in H2. assumption. 
  -inversion H; subst. assumption.
  -inversion H; subst; inversion H0. +apply T_0. +inversion H3. assumption.
  -inversion H; subst. apply T_Pred. apply IHstep in H2. assumption.
  -inversion H; subst. apply T_True.
  -inversion H; subst; inversion H0;subst; apply T_False.
  -inversion H; subst. apply T_Iszero. apply IHstep in H2. assumption.
Qed.
...
Theorem subject_expansion:
  ...
  ~ (forall t t' T, t --> t' /\ |-- t' \in T -> |-- t \in T).
Proof.
  right. unfold not. intros. assert (|-- <{if false then 0 else true}> \in Bool). {apply H with <{true}>. auto. }
...
(** **** Exercise: 2 stars, standard (variation1)
   ...
   counterexample.
      - Determinism of [step]
            (* 1, not changing the stepping rule. *)
      - Progress
            (* 0  scc fls is not a value and cannot be stepped further*)
      - Preservation
            (* 1 *)
*)
(* Do not modify the following line: *)
...
(** **** Exercise: 2 stars, standard, optional (variation3)
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            (* 0,         test fls (iszro zro) fls can be reduced in two ways:
                          - fls (by ST_TestFls)
...
(** **** Exercise: 2 stars, standard, optional (variation4)
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            (* 1 *)
      - Progress
            (* 1  [prd fls] is not well-typed based on existing
                  typing rules. Therefore it doesn't satisfy the progress's typing
                  condition.*)
      - Preservation
            (* 1  [prd fls] is not well-typed *)
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (variation5)
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            (* 1 *)
      - Progress
            (* 0 [test zro tru fls] is well-typed but irreducible.*)
      - Preservation
            (* 1 *)
*)
Module variation5.
...
(** **** Exercise: 2 stars, standard, optional (variation6)
   ...
   this rule?  For each one that does, give a counter-example.
      ...
      - Progress
            (* 1 *)
      - Preservation
            (* 0 [|- test (prd zro) zro zro \in Nat] is well typed,
...
Inductive big_step : tm -> tm -> Prop :=
  | BST_Id : forall t,
    value t -> t ==> t
  | BST_IfTru : forall t1 t2 t3 t2',
    t1 ==> <{true}> ->
    t2 ==> t2' ->
    <{ if t1 then t2 else t3 }> ==> t2'
  | BST_IfFls : forall t1 t2 t3 t3',
    t1 ==> <{false}>->
    t3 ==> t3' ->
    <{ if t1 then t2 else t3 }> ==> t3'
  | BST_Succ : forall t1 t1',
      t1 ==> t1' ->
  ...
  | BST_Pred0 :
      <{ pred 0 }> ==> <{ 0 }>
  | BST_PredSucc : forall t t',
      t ==> t' ->
      (prd (scc t)) ==> t'
  | BST_Iszero : forall t,
      t ==> <{0}> ->
      <{ iszero t }> ==> <{ true }>
  | BST_IszeroSucc : forall t n,
      t ==> <{ succ n }> ->
      <{iszero t }> ==> <{false}>
...
Definition bigstep_preservation:= forall t t' T,
  |-- t \in T ->
  t ==> t' ->
  |-- t' \in T.
(* Do not modify the following line: *)
