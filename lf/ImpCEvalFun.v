...
(** **** Exercise: 2 stars, standard, optional (peven)
    ...
    program. *)
Fixpoint evenb(n:nat):bexp :=
  match n with
  |O=><{true}>  
  |S O=><{false}>
  |S (S n')=>evenb n'
  end.
Definition pup_to_n1 (st:state) : com:=
   <{if (evenb (aeval st (AId X))) then Z:=0 else Z:=1 end}>.
Compute test_ceval (X !-> 5) (pup_to_n1 (X !-> 5)).
Example pup_to_n_2 :
   test_ceval (X !-> 4) (pup_to_n1 (X !-> 4))
   = Some (4, 0, 0).
Proof. reflexivity. Qed.
...
Proof.
  ...
  +destruct IHHce1 as [i1 H1]. destruct IHHce2 as [i2 H2].
    exists (S (i1+i2)). simpl. apply ceval_step_more with (i2:=i1+i2) in H1. rewrite H1.
    apply ceval_step_more with (i2:=i1+i2) in H2. rewrite H2. reflexivity. lia. lia.
  +destruct IHHce as [i1 H1].  exists (S (i1)). simpl. rewrite H. rewrite H1. reflexivity.
  +destruct IHHce as [i1 H1]. exists (S (i1)). simpl. rewrite H. rewrite H1. reflexivity.
  +exists 1. simpl. rewrite H. reflexivity.
  +destruct IHHce1 as [i1 H1]. destruct IHHce2 as [i2 H2].
   exists (S (i1+i2)). simpl. apply ceval_step_more with (i2:=i1+i2) in H1. rewrite H1.
    apply ceval_step_more with (i2:=i1+i2) in H2. rewrite H2. rewrite H. reflexivity. lia. lia.
Qed.
(** [] *)
