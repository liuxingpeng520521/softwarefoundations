...
Proof.
  intros.
  extensionality x.
  unfold union. apply Nat.add_assoc.
  Qed.
...
Proof.
  intros.
  extensionality x.
  unfold union. apply Nat.add_comm.
  Qed.
...
Proof.
  intros. rewrite union_comm with (a:=a) (b:=c).
  rewrite union_assoc with (a:=b) (b:=c) (c:= a).
  rewrite union_comm with (a:=a) (b:=(union b c)).
  reflexivity.
  Qed.
...
Proof.
  ...
  +bdestruct (x<=?a).
   -reflexivity.
   -assert(contents (a::insert x l)=contents (a::x::l)).
    *simpl. rewrite IHl. reflexivity.
    *rewrite H0. rewrite union_assoc with (a:=singleton x) (b:=singleton a) (c:=contents l).
     rewrite union_comm with  (a:=singleton x) (b:=singleton a).
     rewrite<-union_assoc with (a:=singleton a) (b:=singleton x) (c:=contents  l).
     reflexivity.
   Qed.
...
Proof.
  intros. induction l.
  +reflexivity.
  +specialize H with a. simpl in H. unfold union in H. unfold singleton in H.
...
Lemma contents_cons_inv : forall l x n,
    ...
    exists l1 l2,
      ...
      /\ contents (l1 ++ l2) x = n.
Proof.
  intros l. induction l;simpl;intros.
  +discriminate H.
...
Proof.
  ...
  +destruct bl.
   -reflexivity.
   -specialize H with v. simpl in H. unfold union in H. unfold singleton in H.
   rewrite Nat.eqb_refl in H. inversion H.
