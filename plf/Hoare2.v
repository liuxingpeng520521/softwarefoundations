...
Theorem parity_outer_triple_valid : forall m,
  ...
  +destruct(st X) eqn:E.
   -discriminate.
   -destruct n eqn :F. * discriminate. *lia.
  +destruct(st X) eqn:E.
  -lia.
  -destruct n; lia.
...
Proof. verify.
   +destruct (st X).
    -lia.
    -rewrite<-H. simpl. destruct(n); simpl; lia.
   +rewrite<-H. simpl. lia.
 Qed.
(** [] *)
...
Proof. verify.
   +symmetry in H0. apply andb_true_eq in H0. destruct H0. symmetry in H. rewrite negb_true_iff in H.
    rewrite eqb_neq in H. assumption.
   +symmetry in H0. apply andb_true_eq in H0. destruct H0. symmetry in H0. rewrite negb_true_iff in H0.
   rewrite eqb_neq in H0. assumption.
   + apply andb_false_iff in H0. destruct H0.
    -left. rewrite negb_false_iff in H. rewrite eqb_eq in H. assumption.
    -right. rewrite negb_false_iff in H. rewrite eqb_eq in H. assumption.
Qed.
(** [] *)
...
Proof. verify. 
   +assert ((pow2 (st X) + (pow2 (st X) + 0))=(pow2 (st X) + pow2 (st X) + 0)).
   { rewrite plus_assoc. reflexivity. }
   rewrite H. rewrite<-pow2_plus_1.
   assert (pow2 (st X + 1) + (pow2 (st X + 1)) = pow2 (st X + 1 + 1) ).
   {
    remember (st X) as m. rewrite<-pow2_plus_1. reflexivity.
   }
   rewrite<-H0. lia.
...
Proof.
  unfold is_wp. split.
  +eapply  hoare_consequence_pre. -apply hoare_asgn. -intros st H. unfold assn_sub. simpl. rewrite t_update_eq. simpl in H. lia.
  +intros. unfold hoare_triple in H.
  unfold assert_implies. intros. simpl in *.
  assert(st=[X:=Y+1]=>(X!->(st Y+1);st)). {constructor. reflexivity. }
  apply H in H1.
   -rewrite t_update_eq in H1. simpl in H1. lia. - assumption.
Qed.
(** [] *)
...
Definition dfib (n : nat) : decorated :=
  <{
    ...
    while X <> 1 + n do
                  ...
                  {{ X+1>0/\Z = ap fib (1+X-1)/\Z+Y = ap fib (1+X) }}
      T := Z
                  {{ X+1>0/\T = ap fib (1+X-1)/\Z+Y = ap fib (1+X) }};
      Z := Z + Y
                  {{ X+1>0/\T = ap fib (1+X-1)/\Z = ap fib (1+X)  }};
      Y := T
                  {{X+1>0/\Y = ap fib (1+X-1)/\Z = ap fib (1+X) }};
      X := 1 + X
                  {{X>0/\Y = ap fib (X-1)/\Z = ap fib (X) }}
    end
    {{X>0/\Y = ap fib (X-1)/\Z = ap fib (X)/\ X=1+n}} ->>
...
Proof. verify. 
+destruct(st X); reflexivity.
+destruct(st X). -inversion H. -assert(S n0-1=n0). {lia. } rewrite H0. reflexivity.
+assert(S n-1=n). {lia. } rewrite H0. reflexivity.
Qed.
(** [] *)
