...
Theorem add_inc_count : forall (n:nat) (s:bag), count n (add n s)=1+count n s.
Proof.
    intros. simpl. assert(n=?n=true). {induction n. +reflexivity. +simpl. rewrite IHn. reflexivity. }
...
Theorem eqblist_refl : forall l:natlist,
  ...
  Proof. 
    intros l. induction l as [|n l' IHl'].
    -simpl. reflexivity.
    -simpl. rewrite<-eqb_eq. rewrite<-IHl'. reflexivity.
  Qed. 
...
Proof.
  intros. rewrite H. rewrite<-H0. rewrite<-H. reflexivity. 
Qed.
(** [] *)
...
Proof. intros x.
destruct x as [x'].
simpl. rewrite<- NatList.eqb_eq. reflexivity.
Qed.
(** [] *)
