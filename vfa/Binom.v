...
Proof.
intros. induction t; simpl in H; try easy. induction t2; try easy. induction u; simpl in H0; try easy.
induction u2; try easy. simpl. bdestruct (k >? k0); simpl; split; try lia; split; try easy.
Qed.
(** [] *)
...
Proof.
unfold priq,insert. intros. apply carry_valid; try easy. right. simpl. easy.
Qed.
(** [] *)
...
| priqueue_elems_cons : forall qh qt lh lt l,
             tree_elems qh lh ->
             priqueue_elems qt lt ->
             Permutation l (lh++lt) ->
             priqueue_elems (qh::qt) l.
...
Proof.
intros. induction t.
- inversion H0. subst. apply tree_elems_node with bl br; try easy.
  apply Permutation_sym in H. apply perm_trans with e1; easy.
- inversion H0. subst. apply Permutation_nil in H. subst. easy.
Qed.
(** [] *)
...
Proof.
intros. induction q.
* inv H0. apply Permutation_nil in H. subst. apply priqueue_elems_nil.
* inv H0. apply priqueue_elems_cons with lh lt; try easy. apply Permutation_sym in H.
   apply Permutation_trans with e1; easy.
Qed.
...
Proof.
intros. induction t.
- destruct IHt1,IHt2. exists (k::x++x0). apply tree_elems_node with x x0; easy.
- exists []. constructor.
Qed.
