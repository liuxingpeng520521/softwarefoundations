...
Theorem hoare_post_true : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros. unfold hoare_triple. intros. apply H. Qed.
(** [] *)
...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  unfold hoare_triple. intros. unfold not in H. apply H in H1. contradiction. Qed.
(** [] *)
...
Theorem hoare_asgn_fwd :
  ...
  {{fun st => P (X !-> m ; st)
           /\ st X = aeval (X !-> m ; st) a }}.
Proof.
  unfold hoare_triple. intros. inversion H; subst. destruct H0. rewrite t_update_shadow.
   rewrite<-H1. rewrite t_update_same. rewrite t_update_eq. auto. 
Qed.
(** [] *)
...
Proof.
  ...
  exists (st X). rewrite t_update_shadow.
...
Proof. 
  ...
  apply hoare_consequence_pre
    with (P' := (X = 1) [X |-> 1]).
  - apply hoare_asgn.
  - unfold "->>"  , assn_sub, t_update. intros.
     simpl. reflexivity.
Qed.
...
Proof.
  ...
  - apply hoare_asgn.
  - unfold "->>". unfold assn_sub. unfold t_update.
...
Definition swap_program : com:= <{Z:= X; X:= Y; Y:= X}>.
...
Proof.
  unfold hoare_triple.
  intros H.
  specialize H with (a := X) (n := 5).
  simpl in H.
  assert( (Y!->3; X!->3; X!->5) Y=5). { apply H with(st:=(X!->5)).
    +apply E_Seq with (X!->3;X!->5).  -apply E_Asgn. reflexivity. -apply E_Asgn. reflexivity.
    +reflexivity.  }
  assert( (Y!->3; X!->3; X!->5) Y=3). {reflexivity. }
...
Proof. 
  apply hoare_if1.
  +simpl. intros st st' H [H1 H2]. inversion H; subst. rewrite t_update_eq. rewrite t_update_neq.
   -assumption.
   -unfold not. intro. discriminate.
  +simpl. intros st st' H [H1 H2]. inversion H; subst.
    destruct (eqb (st' Y ) 0) eqn:R.
    -rewrite eqb_eq in R. rewrite R in H1. lia.
    -unfold not in H2. rewrite negb_true_iff in H2. contradiction. 
  Qed.
End If1.
...
Proof.
  apply E_Repeat with(st':=(Y !-> 1 ; X !-> 1)).
  +apply E_Seq with (X !-> 1). -apply E_Asgn. reflexivity. - apply E_Asgn. reflexivity.
...
Theorem hoare_repeat : forall P Q R (b:bexp) c,
    {{P}} c {{Q}} ->
    {{Q}} while ~b do c end {{R}}->
    {{P}} repeat c until b end {{R}}.
  Proof.
    intros. intros st st' H1 H2.  inversion H1; subst.
...
Theorem hoare_repeat_test :
  ...
  {{ X = 0 /\ Y > 0 }}.
Proof. 
  apply hoare_repeat with (Y>0)%assertion.
  +unfold hoare_triple. intros. inversion H; subst.
   inversion H4; subst. inversion H6; subst. simpl in *.
   rewrite t_update_neq. -rewrite t_update_eq. assumption.
                         -unfold not. intro. discriminate.
  +unfold hoare_triple. intros. 
  remember (<{while ~ X = 0 do Y := X; X := X - 1 end}>) as E.
  induction H; inversion HeqE; subst.
   -split.
    *simpl in H. apply eqb_eq. apply negb_false_iff. assumption.
    *assumption.
   -apply IHceval2. 
    *assumption.
    *inversion H1;subst. inversion H6; subst. inversion H8; subst. simpl in *.
     rewrite t_update_permute.
     **rewrite t_update_eq.
      rewrite negb_true_iff in H. rewrite eqb_neq in H. lia.
...
Proof.
  ...
  - apply hoare_havoc.
  - unfold assert_implies. unfold assn_sub. unfold havoc_pre. intros. simpl. exists (st X).
...
Theorem assert_assume_differ : exists (P:Assertion) b (Q:Assertion),
  ...
  /\ ~ ({{P}} assert b {{Q}}).
Proof.
  exists True ,BFalse, False.
  split.
  +unfold hoare_triple. intros. inversion H; subst. simpl in *. discriminate.
  +unfold not. unfold hoare_triple. intros. simpl in *.
  assert(exists st' : state, RError = RNormal st' /\ False).
  {apply H with (X!->0).
...
Theorem assert_implies_assume : forall P b Q,
  ...
  -> ({{P}} assume b {{Q}}).
Proof.
  unfold hoare_triple. intros. inversion H0; subst. exists st.
  split.
  +reflexivity.
  +
   assert(exists st' : state, RNormal st = RNormal st' /\ Q st').
   {apply H with st. +apply E_AssertTrue. assumption. +assumption. }
   destruct H2. destruct H2. injection H2 as E. rewrite E. assumption.
Qed.
...
Proof. intros. unfold hoare_triple. intros.
  ...
  +split. -reflexivity. - destruct H0. apply H0.
  +destruct H0. assert (beval st b = true). {assumption. }
