...
(* no, functions are values, evaluation will delay, causing an infinite loop*)
...
apply multi_step with  (<{ (ref (loc 0)):= (\x:Nat, (!(loc 0)) x) }> , <{ \x:Nat, (!(loc 1)) x }>::nil ).
+apply ST_Assign1. apply ST_Ref. apply ST_RefValue. auto.
+apply multi_step with (<{ (loc 1) := (\x:Nat, (!(loc 0)) x) }>,(<{ \x:Nat, (!(loc 1)) x }> :: nil )++<{loc 0}>:: nil).
 -apply ST_Assign1. apply ST_RefValue. auto.
 -apply multi_step with (<{ unit }> , (<{ \x:Nat, (!(loc 1)) x }> :: nil) ++ <{ \x:Nat, (!(loc 0)) x }> :: nil).
...
<{ (ref (ref (\x:Nat, (!(loc 1)) x))):= (\x:Nat, (!(loc 0)) x)}> / nil
    --><{ref (\x:Nat, (!(loc 1)) x)}>/ nil --><{loc 0}> / (<{ \x:Nat, (!(loc 1)) x }>)::nil
<{ (ref (loc 0)):= (\x:Nat, (!(loc 0)) x) }> / (<{ \x:Nat, (!(loc 1)) x }>)
    --><{ref v}>/ (<{ \x:Nat, (!(loc 1)) x }>) --> <{loc 1}>/(<{ \x:Nat, (!(loc 1)) x }> ++ V :: nil).
<{ (loc 1) := (\x:Nat, (!(loc 0)) x) }> /  (<{ \x:Nat, (!(loc 1)) x }> ++ V :: nil).
    --> <{ unit }> / replace 1 (\x:Nat, (!(loc 0)) x) (<{ \x:Nat, (!(loc 1)) x }> ++ V :: nil).
<{ unit }> / (<{ \x:Nat, (!(loc 1)) x }> ++ <{ \x:Nat, (!(loc 0)) x }> :: nil).
*)
...
Theorem store_not_unique:
  exists st, exists ST1, exists ST2,
    ...
    ST1 <> ST2.
Proof.
  exists (<{!(loc 0)}> :: nil). exists (<{Nat}> :: nil). exists (<{Unit}> :: nil).
  split.
    - constructor; auto.
...
(** **** Exercise: 4 stars, standard (factorial_ref)
    ...
    [4].) *)
Definition fac := <{\x:Nat, if0 x then 1 else x*((!r) (pred x))}>.
Definition factorial : tm:=
<{(\y:Nat, ((\r:Ref (Nat->Nat), ((r:=fac);((!r) y))) (ref (\x:Nat,1))))}>.
Lemma factorial_type : empty; nil |-- factorial \in (Nat -> Nat).
Proof with eauto.
   unfold factorial. unfold fac. 
   eapply T_Abs...
   eapply T_App...
    ...
    +eapply T_Assign.
     eapply T_Var. rewrite update_eq. reflexivity.
   eapply T_Abs. eapply T_If0. 
     -eapply T_Var. rewrite update_eq. reflexivity.
     -auto.
     -eapply T_Mult. *eapply T_Var. reflexivity.
                     *eapply T_App. eapply T_Deref.
                       ++eapply T_Var. reflexivity.
                       ++eapply T_Pred. eapply T_Var. reflexivity.
Qed. 
...
(*See my solution in ReferencesLastExercises.v.*)
