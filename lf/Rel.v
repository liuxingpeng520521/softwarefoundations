...
Proof. unfold not. unfold partial_function. intros.
assert (0 = 1) as Nonsense. {
  apply H with (x := 0).
  - apply tr_zero.
  - apply tr_zero. }
...
Proof.
  Proof. unfold partial_function. intros.
  inversion H. Qed.
(** [] *)
...
Proof.
  unfold lt. unfold transitive.
  intros n m o Hnm Hmo. apply le_S in Hnm. 
  induction o as [| o']. 
...
Proof.
  ...
  assert (1<=0) as Nonsense. {
     apply H with (b:=1) (a:=0).
     apply le_S. apply le_n.
  }
  apply le_Sn_n in Nonsense. apply Nonsense.
Qed.
...
Proof.
  ...
  - symmetry in H. rewrite H. reflexivity. Qed.

Theorem le_antisymmetric :
  antisymmetric le.
Proof.
  unfold antisymmetric. intro a.
  induction a.
  +intros. inversion H0. reflexivity.
  +intros. induction b.
   -inversion H.
   -apply plus_1. apply Sn_le_Sm__n_le_m in H. apply Sn_le_Sm__n_le_m in H0.
    apply IHa. apply H. apply H0.
Qed.
...
Theorem le_step : forall n m p,
  ...
  n <= p.
Proof.
  intros. apply Sn_le_Sm__n_le_m. apply le_trans with (a := (S n)) (b := (m)) (c := S p).
  -assert (n<m->S n<=m). {unfold lt. intro. apply H1. }
   apply H1. apply H.
  -apply H0.
Qed.
