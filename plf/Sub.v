...
(** **** Exercise: 2 stars, standard (proper_subtypes)
    ...
    forall T,
         ...
         exists S,
            S <: T  /\  S <> T
0  (T1, T2) doesn't have a proper subtype.
            *)
...
Proof with eauto. apply S_Arrow...  apply S_Prod... Qed.
...
Proof with eauto. apply S_Arrow... Qed.
(** [] *)
...
(* empty |-- ((\z:A.z), (\z:B.z)) \in (A->A * B->B) *)
    ...
    empty |-- ((\z:A,z), (\z:B,z)) \in ((A->A) * (B->B)).
    Proof. intros. apply T_Pair; apply T_Abs; apply T_Var; apply update_eq. Qed. 
...
(* empty |-- (\x:(Top * B->B). x.snd) ((\z:A.z), (\z:B.z))
    ...
    Proof. intros. eapply T_App.
     + apply T_Abs. eapply T_Snd. apply T_Var. rewrite update_eq. reflexivity.
     + apply T_Pair. -eapply T_Sub. *apply T_Abs. apply T_Var; apply update_eq.
                                    *apply S_Top.
                     -apply T_Abs. apply T_Var; apply update_eq. Qed. 
...
(* empty |-- (\z:(C->C)->(Top * B->B). (z (\x:C.x)).snd)
    ...
    empty |-- (\z:(C->C)->(Top * (B->B)), (z (\x:C,x)).snd)
         ...
         \in (B->B) .
    Proof. intros. eapply T_App.
     + apply T_Abs. eapply T_Snd. eapply T_App. -apply T_Var. rewrite update_eq. reflexivity.
                                                -apply T_Abs. apply T_Var. rewrite update_eq. reflexivity.
     + apply T_Abs. apply T_Pair. -eapply T_Sub. *apply T_Abs. apply T_Var; apply update_eq.
                                    *apply S_Top.
                     -apply T_Abs. apply T_Var; apply update_eq. Qed. 
End Examples2.
...
Proof with auto.
  intros U Hs.
  remember <{Bool}> as V. 
  induction Hs; inversion HeqV;subst...
  apply IHHs2 in H. remember H as He. clear HeqHe. apply IHHs1 in H. rewrite H...
Qed.
(** [] *)
...
Proof with eauto.
  ...
  induction Hs;intros; try(inversion HeqV;subst)...
  apply IHHs2 in H. destruct H as [T1' [T2' [H1' [H2' H3']]]].
...
Lemma sub_inversion_Prod : forall U V1 V2,
     ...
     exists U1, exists U2,
       U = <{U1 *U2}> /\ (U1 <: V1) /\ (U2 <: V2).
Proof with eauto.
  intros U V1 V2 Hs. remember <{V1*V2}> as V.
  generalize dependent V2. generalize dependent V1.
  induction Hs; intros; try(inversion HeqV;subst)...
  +apply IHHs2 in H. destruct H as [T1' [T2' [H1' [H2' H3']]]].
  apply IHHs1 in H1'. destruct H1' as [T1'' [T2'' [H1'' [H2'' H3'']]]].
...
Proof with auto.
  ...
  remember <{Top}> as V.
  induction Hs; inversion HeqV;subst...
  apply IHHs1 in H. remember H as He. clear HeqHe. apply IHHs2 in H. rewrite H...
Qed.
...
Proof with eauto.
    ...
    try(inversion HeqT)...
    +rewrite HeqT in H1. apply sub_inversion_arrow in H1.
    destruct H1 as [V1 [V2 [H4 H5]]]... rewrite H2. eapply IHhas_type...
    +rewrite HeqT in H1. apply sub_inversion_arrow in H1.
    destruct H1 as [V1 [V2 [H4 H5]]]... rewrite H2. eapply IHhas_type...
    +rewrite HeqT in H1. apply sub_inversion_arrow in H1.
    destruct H1 as [V1 [V2 [H4 H5]]]... rewrite H2. eapply IHhas_type...
    +rewrite HeqT in H1. apply sub_inversion_arrow in H1.
    destruct H1 as [V1 [V2 [H6 H7]]]... rewrite H4. eapply IHhas_type...
Qed.
...
Lemma canonical_forms_of_Prod : forall Gamma s T1 T2,
  Gamma |-- s \in (T1 *T2) ->
  value s ->
  exists t1, exists t2, s = <{(t1,t2)}> /\ value t1 /\ value t2.
Proof with eauto.
  ...
  exists t1...
  + subst. apply sub_inversion_Prod in H1.
...
Proof with eauto.
  ...
  -(*Pair*)
   destruct IHHt1...
    +destruct IHHt2... destruct H0...
    +destruct H...
  -destruct IHHt...
   +apply canonical_forms_of_Prod in Ht; [|assumption].
    destruct Ht as [t1 [t2 [H0[H1 H2]]]].  right. rewrite H0. exists t1...
   +destruct H...
  -destruct IHHt...
    +apply canonical_forms_of_Prod in Ht; [|assumption].
      destruct Ht as [t1 [t2 [H0[H1 H2]]]].  right. rewrite H0. exists t2...
    +destruct H...
Qed.
...
Lemma typing_inversion_var : forall Gamma (x:string) T,
  ...
  exists S,
    Gamma x = Some S /\ S <: T.
Proof with eauto.
  intros Gamma x T H. remember  (tm_var x) as t. induction H;inversion Heqt;subst.
  +exists T1...
  +apply IHhas_type in H1. destruct H1 as [S2 [Hsub Hty]]...
Qed.
...
Lemma typing_inversion_pair : forall Gamma T t1 t2,
  Gamma |-- <{(t1 ,t2)}> \in T ->
  (exists S1, exists S2, <{S1*S2}> <: T
              /\ Gamma |-- t1 \in S1 /\ Gamma |-- t2 \in S2).
Proof with eauto.
  intros. remember <{(t1, t2)}> as tp.
  induction H; 
  inversion Heqtp; subst.
  + exists T1...
  +apply IHhas_type in H1. destruct H1 as [V1 [V2 [H1 H2]]]...
Qed.
...
Lemma prod_types : forall S1 S2 T1 T2,
       ...
       S1 <: T1 /\ S2 <: T2.
Proof with eauto.
  intros. remember <{S1* S2}> as S. remember <{T1 *T2}> as T.
  generalize dependent T1. generalize dependent T2.
...
Proof with eauto.
  ...
  destruct (typing_inversion_pair _ _ _ _ HT)  as [V [F [G [H3 H4]]]].
  destruct (prod_types _ _ _ _ G)...
  -inversion HE;subst... inversion HT;subst...
  destruct (typing_inversion_pair _ _ _ _ HT)  as [V [F [G [H3 H4]]]].
  destruct (prod_types _ _ _ _ G)...
Qed.
...
Preservation, unit --> (\x:Top. x)
    - Suppose we add the following subtyping rule:
                               ...
                               ----------------          (S_Funny3)
                               Unit <: Top->Top
Progress, |-- unit unit \in Top but cannot progress.
    - Suppose we add the following subtyping rule:
...
neither
    - Suppose we add the following reduction rule:
                             ...
                             ---------------------      (ST_Funny5)
                             (unit t) --> (t unit)
neither [unit t] is never well-typed.
...
both  [unit unit] is now well-typed (has type Top), but cannot step into anything.
      [unit true] has type [Top] and can step into [true unit] which is not well-typed.
    - Suppose we _change_ the arrow subtyping rule to:

                          S1 <: T1 S2 <: T2
                          -----------------              (S_Arrow')
                          S1->S2 <: T1->T2
Preservation  [(\x:Bool, if x then true else true) unit] is well
...
Theorem formal_subtype_instances_tf_1e:
  TF (forall S T U V, S <: T -> U <: V ->
         <{(T->T)->U}> <: <{(S->S)->V}>).
Proof.
  right. intro. assert(forall A,<{(Top->Top)->Top}> <: <{(A->A)->Top}>). { intros. apply H;auto. }
  specialize H0 with A. apply sub_inversion_arrow in H0. destruct H0 as [U1 [U2 [H00 [H11 H22]]]].
  injection H00. intros. subst. clear H00 H22. apply sub_inversion_arrow in H11.  destruct H11 as [U1 [U2 [H00 [H11 H22]]]].
  injection H00. intros. subst. apply sub_inversion_Top in H11. discriminate. Qed.
(** [] *)
...
Theorem formal_subtype_instances_tf_1g:
  TF (forall S T U V, S <: T -> U <: V ->
         <{S*V}> <: <{T*U}>).
Proof.
  right. intro. assert(forall S U, <{S*Top}> <: <{Top*U}>). {intros. apply H;auto. }
  specialize H0 with (S:=A) (U:=B). apply sub_inversion_Prod in H0. destruct H0 as [U1 [U2 [H00 [H11 H22]]]].
  injection H00. intros. subst. apply sub_inversion_Top in H22. discriminate. Qed. 
(** [] *)
...
Theorem formal_subtype_instances_tf_2a:
  TF (forall S T,
         ...
         <{S->S}> <: <{T->T}>).
Proof.
  right. intro. assert(forall S, <{S->S}> <: <{Top->Top}>). {intros. apply H. auto. }
  specialize H0 with A. apply sub_inversion_arrow in H0. destruct H0 as [U1 [U2 [H00 [H11 H22]]]].
  injection H00. intros. subst. apply sub_inversion_Top in H11. discriminate. Qed.
(** [] *)
...
Theorem formal_subtype_instances_tf_2b:
  TF (forall S,
         ...
         exists T,
           S = <{T->T}> /\ T <: A).
Proof.
  right. intro. specialize H with <{Top->A}>.
  assert(<{Top->A}><:<{A->A}>) by auto. apply H in H0. destruct H0 as [T [H00 H11]].
  injection H00. intros. subst. discriminate. Qed.
...
(** **** Exercise: 2 stars, standard, optional (formal_subtype_instances_tf_2d)
    ...
    derviation). *)
Lemma not_exists_subtype_S : 
  forall S, ~ (S <: <{S->S}>).
Proof. intros. intro. induction S.
+apply sub_inversion_Top in H. inversion H.
+apply sub_inversion_arrow in H. destruct H as [U1 [U2 [H00 [H11 H22]]]]. inversion H00.
+apply sub_inversion_arrow in H. destruct H as [U1 [U2 [H00 [H11 H22]]]]. inversion H00.
+apply sub_inversion_arrow in H. destruct H as [U1 [U2 [H00 [H11 H22]]]]. inversion H00;subst.
 assert(<{U2}><:<{U1}>). {eapply S_Trans. -apply H22. -assumption. }
 assert(<{U1->U2}><:<{U2->U2}>). {apply S_Arrow;auto. }
 assert(<{U2}><:<{U2->U2}>). {eapply S_Trans. -apply H22. -auto. }
 apply IHS2. apply H1.
+apply sub_inversion_arrow in H. destruct H as [U1 [U2 [H00 [H11 H22]]]]. inversion H00;subst.
+apply sub_inversion_arrow in H. destruct H as [U1 [U2 [H00 [H11 H22]]]]. inversion H00;subst.
Qed.
...
Theorem formal_subtype_instances_tf_2d:
  TF (exists S,
         S <: <{S->S}>).
Proof.
 right. intro. destruct H. apply not_exists_subtype_S in H. auto. Qed. 
(** [] *)
...
Proof.
  right. intro. destruct H. remember H as H0. clear HeqH0.
  specialize H with <{Bool}>. apply sub_inversion_Bool in H. 
  specialize H0 with <{Unit}>. apply sub_inversion_Unit in H0.
  rewrite H in H0. inversion H0. Qed. 
(** [] *)
...
Proof.
  right. intro. destruct H as [T1 [T2 H]]. remember H as H0. clear HeqH0.
  specialize H with <{Bool}> <{Bool}>. apply sub_inversion_Prod in H. destruct H as [U1 [U2 [H00 [H11 H22]]]]. 
  apply sub_inversion_Bool in H11. apply sub_inversion_Bool in H22. inversion H00. subst. 
  specialize H0 with <{Unit}> <{Unit}>. apply sub_inversion_Prod in H0. destruct H0 as [U1 [U2 [H000 [H111 H222]]]]. 
  apply sub_inversion_Unit in H111. apply sub_inversion_Unit in H222. inversion H000. subst.
  inversion H1. Qed. 
(** [] *)
...
Proof.
  right. intro. destruct H as [T1 [T2 H]]. remember H as H0. clear HeqH0.
  specialize H with <{Bool}> <{Bool}>. apply sub_inversion_arrow in H. destruct H as [U1 [U2 [H00 [H11 H22]]]]. 
  inversion H00. subst. apply sub_inversion_Bool in H11. subst. 
  specialize H0 with <{Unit}> <{Unit}>. apply sub_inversion_arrow in H0. destruct H0 as [U1 [U2 [H000 [H111 H222]]]]. 
  inversion H000. subst. apply sub_inversion_Unit in H111. inversion H111. Qed. 
(** [] *)
...
Proof.
  right. intro. destruct H as [T1 [T2 H]]. remember H as H0. clear HeqH0.
  specialize H with <{Bool}> <{Bool}>. apply sub_inversion_arrow in H. destruct H as [U1 [U2 [H00 [H11 H22]]]]. 
  inversion H00. subst. apply sub_inversion_Bool in H22. subst. 
  specialize H0 with <{Unit}> <{Unit}>. apply sub_inversion_arrow in H0. destruct H0 as [U2 [U3 [H000 [H111 H222]]]]. 
  inversion H000. subst. apply sub_inversion_Unit in H222. inversion H222. Qed. 
(** [] *)
...
Proof.
  left. exists f. split.
  +intros. intro. generalize dependent j. induction i.
   -intros. induction j. *auto. *simpl in *. inversion H0.
   -intros. induction j. *simpl in *. inversion H0.
                         *simpl in *. inversion H0. rewrite H2 in *.
                          specialize IHi with j. apply IHi.
                          ++auto.
                          ++auto.
  +intros. simpl. induction i.
   -simpl. auto.
   -simpl. apply S_Arrow;auto.
Qed. 
(** [] *)
...
Fixpoint f' (n:nat) :ty := 
                             match n with 
                             | 0=>Ty_Arrow  Ty_Top Ty_Top
                             | S n'=> (Ty_Arrow (Ty_Arrow (f' n') Ty_Top) Ty_Top)
                             end. 
...
Theorem formal_proper_subtypes:
  TF (forall T,
         ...
         exists S,
           S <: T /\ S <> T).
Proof.
  right. intro. specialize H with <{Bool*Bool}>. 
  assert(~(<{Bool*Bool}> = <{Bool}> \/ (exists n, <{Bool*Bool}> = <{Base n}>) \/ <{Bool*Bool}> = <{Unit}>)).
  {intro. destruct H0.
   +inversion H0.
   +destruct H0. -inversion H0. inversion H1. -inversion H0. }
  apply H in H0. destruct H0. destruct H0.
  apply sub_inversion_Prod in H0. destruct H0 as [U1 [U2 [H00 [H11 H22]]]].
  subst. apply sub_inversion_Bool in H11 ,H22. subst. auto.
Qed.
(** [] *)
...
Proof.
  ...
  - (* Pair *)
    apply sub_inversion_Prod in H2.
    destruct H2 as [U1 [U2 [Heq [HU1 HU2]]]].
    inversion Heq. subst.
    rewrite IHsubtype1; auto.
    rewrite IHsubtype2; auto.
...
Proof.
  ...
  +intros. destruct H.  apply sub_antisymmetry in H.
   -rewrite<-H. apply T_App with <{(A->A)*Unit}>.
    *assert (forall Gamma t,Gamma|--t\in ((A->A)*Top ->A->A)->
             Gamma|--t\in ((A->A)*Unit ->A->A)).
    {intros. eapply T_Sub. +apply H1. +auto. }
    apply H1. apply T_Abs. eapply T_Fst. apply T_Var. apply update_eq.
    *auto.
   -auto.
  +intros. split.
   -assert( <{(\p:T*Top,(p.fst)) (\z:A,z,unit)}> -->* <{\z:A,z}>) by normalize.
   assert(empty |--\z:A,z \in <{A->A}> ). {auto. }

   apply canonical_forms_of_arrow_types in H.
    *destruct H as [x [S1 [s2 H00]]].
     inversion H00;subst.
    * Admitted.
(** [] *)
