...
Theorem andb_eq_orb :
  ...
  (andb b c = orb b c) ->
  b = c.
Proof. destruct b;simpl;intros; rewrite H; reflexivity.
Qed.


Theorem add_assoc : forall n m p : nat,
    n + (m + p) = (n + m) + p.
Proof. induction n;simpl;intros;try (rewrite IHn);
reflexivity. Qed.
...
Proof. induction lst1;simpl; 
try(intros; destruct x;simpl;rewrite IHlst1); reflexivity. Qed.
...
Theorem add_assoc' : forall n m p : nat,
    n + (m + p) = (n + m) + p.
Proof. induction n;[idtac|simpl;intros;rewrite IHn];
reflexivity. Qed.
(** [] *)
...
Proof.   repeat (apply ev_SS; try(apply ev_0) ).
Qed.
...
Proof.
  ...
  -apply MEmpty.
  -apply MChar.
  -destruct re1; try(destruct re2; try(apply MApp; [apply IH1|apply IH2]);
  inversion IH2; rewrite app_nil_r; apply IH1);
  inversion IH1; simpl; destruct re2; apply IH2.
  -destruct re1; try(destruct re2; try(apply MUnionL); apply IH);
   inversion IH.
  -destruct re1;try(apply IH);try(destruct re2; try(apply MUnionR; apply IH); inversion IH).
  -destruct re; try(apply MEmpty);try(apply MStar0).
  -destruct re; try(apply star_app; [apply MStar1; apply IH1|apply IH2] ); try(inversion IH1). 
...
Proof.
  ...
  induction M
    as [| x'
        ...
        | re | s1 s2 re Hmatch1 IH1 Hmatch2 IH2];
  simpl;auto.
  -destruct re1; try(destruct re2; auto;
  inversion IH2; rewrite app_nil_r; auto);
  inversion IH1; simpl; destruct re2; auto.
  -destruct re1; try(destruct re2; auto);
   inversion IH.
  -destruct re1;auto;try(destruct re2; auto; inversion IH).
  -destruct re; auto.
  -destruct re; try(apply star_app; [apply MStar1; auto|auto] ); try(inversion IH1). 
  inversion IH2. simpl. auto.
Qed.
...
Proof.
  ...
  induction Hmatch
    as [ | x | s1 re1 s2 re2 Hmatch1 IH1 Hmatch2 IH2
       | s1 re1 re2 Hmatch IH | re1 s2 re2 Hmatch IH
       | re | s1 s2 re Hmatch1 IH1 Hmatch2 IH2 ];simpl;intros;try(lia).
  (* MApp *)
  -rewrite-> app_length in H. apply add_le_cases in H.
    destruct H as [H1 | H2].
     ...
     *split;auto.
      ...
      rewrite <- (app_assoc T x (napp m x0) x1). auto.
     ...
     *split;auto.
      intros m. rewrite <- app_assoc. auto.
  (* MUninoL *)
  - apply plus_le in H. destruct H as [H1 H2]. apply IH in H1.
    destruct H1 as [x  [x0  [x1  H]]]. destruct H as [H3 [H4 H5]]. 
    exists x. exists x0. exists x1. split;auto.
  (* MUninoR *)
  -apply plus_le in H. destruct H as [H1 H2]. apply IH in H2.
  ...
  -destruct s1;auto.
    ...
    *apply leb_iff in E. apply IH1 in E. destruct E as [x'  [x0  [x1  E]]]. destruct E as [H1 [H2 H3]].
      ...
      **rewrite -> app_assoc. rewrite -> app_assoc. rewrite <- (app_assoc T x' x0 x1).
      rewrite <- H1. reflexivity.
      **split;auto. intros m. rewrite -> app_assoc. rewrite -> app_assoc. rewrite <- (app_assoc T x' (napp m x0) x1). auto.
...
Proof.
  ...
  exists x. exists x0. exists (x1 ++ s2). split;[|split ;[|split]];auto.
    * rewrite -> app_assoc. rewrite -> app_assoc. rewrite <- (app_assoc T x x0 x1).
      rewrite <- H1. reflexivity.
    * apply le_trans with (o := pumping_constant re1 + pumping_constant re2) in H3. auto. 
      apply le_plus_l. 
    * intros m. rewrite -> app_assoc. rewrite -> app_assoc.
      rewrite <- (app_assoc T x (napp m x0) x1). auto.
  (* MApp-2 pumping_constant re2 <= length s2 *)
  -- destruct (pumping_constant re1 <=? length s1) eqn:E.
     (* 和MAdd-1完全一�? pumping_constant re1 <= length s1 *)
     * apply leb_iff in E. apply IH1 in E.  destruct E as (x & x0 & x1 & H). destruct H as (H3 & H4 & H5 & H6).
       exists x, x0, (x1 ++ s2). split;[|split ;[|split]];auto.
     ** rewrite -> app_assoc. rewrite -> app_assoc. rewrite <- (app_assoc T x x0 x1).
        rewrite <- H3. reflexivity.
     ** apply le_trans with (o := pumping_constant re1 + pumping_constant re2) in H5. auto. 
        apply le_plus_l. 
     ** intros m. rewrite -> app_assoc. rewrite -> app_assoc.
        rewrite <- (app_assoc T x (napp m x0) x1). auto.
     ...
     * apply le_false_rev in E. apply IH2 in H2. destruct H2 as (x & x0 & x1 & H). destruct H as (H1 & H2 & H3 & H5).
       exists (s1 ++ x), x0, x1. split;[|split ;[|split]];auto.
     ** rewrite <- app_assoc. rewrite <- H1. reflexivity.
     ** rewrite -> app_length. rewrite <-add_assoc. apply le_plus_both;auto.
     ** intros m. rewrite <- app_assoc. auto.
  (* MUninoL *)
  -  apply plus_le in H. destruct H as [H1 H2]. apply IH in H1.
    destruct H1. destruct H. destruct H. destruct H as [H3 [H4 H5]]. 
    exists x. exists x0. exists x1. split;auto;split;auto;split; destruct H5 as [H6 H7].
    * apply le_trans with (o := pumping_constant re1 + pumping_constant re2) in H6. auto. apply le_plus_l. 
    * intros m. auto.
  (* MUninoR *)
  - apply plus_le in H. destruct H as [H1 H2]. apply IH in H2.
    destruct H2 as (x & x0 & x1 & H). destruct H as (H3 & H4 & H5 & H6). 
    exists x, x0, x1. split;auto; split;auto;split;auto. 
    * apply le_trans with (o := pumping_constant re1 + pumping_constant re2) in H5. auto. 
  ...
  --remember (x::s1) as s. simpl in H.
     ...
     * apply leb_iff in E. apply IH1 in E. destruct E as (x0 & x1 & x2 & Ha & Hb & Hc & Hd).
       ...
       rewrite <- (app_assoc T x0 x1 x2). rewrite <- Ha. reflexivity. split;auto;
       split;auto. intros m. rewrite -> app_assoc. rewrite -> app_assoc. 
       rewrite <- (app_assoc T x0 (napp m x1) x2). auto.
     * apply le_false_rev in E. exists nil, s, s2. simpl. split. reflexivity. split.
     ** unfold not. intros Hfalse. rewrite -> Heqs in Hfalse. discriminate.
     ** split;auto. intros m. apply napp_star; auto.
Qed.
(* Do not modify the following line: *)
...
Proof. intros. destructpf b; destructpf c; destructpf d.
Qed.
...
Ltac destructpf' x := destruct x; try(simpl; auto; reflexivity).
...
Theorem andb_true_elim2' : forall b c : bool,
    andb b c = true -> c = true.
Proof. intros. destructpf' b; destructpf' c.
Qed.
