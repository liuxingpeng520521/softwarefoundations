...
Proof.
  induction c; intros.
  +eapply H_Consequence_post. -eapply H_Skip.
   -intros. reflexivity.
  +eapply H_Consequence_pre. -eapply H_Asgn.
   -intros. reflexivity.
  +eapply H_Consequence_pre. -eapply H_Seq. *apply IHc1. *apply IHc2. 
   -intros. apply H.
  +eapply H_Consequence_pre. -eapply H_If. *apply IHc1. *apply IHc2.
   -intros. apply H.
  +eapply H_Consequence. -eapply H_While. apply IHc.
   -intros.  reflexivity. -intro. reflexivity.
Qed.
...
Proof.
  induction c; intros.
  +eapply H_Consequence_pre. -eapply H_Skip.
   -intros. inversion H.
  +eapply H_Consequence_pre. -eapply H_Asgn.
   -intros. inversion H.
  +eapply H_Consequence_pre. -eapply H_Seq. *apply IHc1. *apply IHc2. 
   -intros. inversion H.
  +eapply H_If.
    -eapply H_Consequence_pre. *apply IHc1. *intros st [H0 H1]. assumption.    
    -eapply H_Consequence_pre. *apply IHc2. *intros st [H0 H1]. assumption.    
  +eapply H_Consequence_post. 
   -eapply H_While. eapply H_Consequence_pre. *apply IHc. *intros st [H0 H1]. assumption. 
...
Proof.
  intros b c Q st st' Hvalid H. inversion H. eauto.
Qed.
...
Proof.
  ...
  +apply H_Seq with (wp c2 Q).
   -eapply IHc1. unfold wp. intros.
    eapply HT. econstructor; eassumption. assumption.
   -eapply IHc2. intros. apply H0; assumption.
  +apply H_If.
   -apply IHc1. intros st st' H1 H2. destruct H2.
    eapply HT; eauto.
   -apply IHc2. intros st st' H1 H2. destruct H2.
    eapply HT; eauto. apply E_IfFalse; auto. 
    unfold bassn in H0. destruct (beval st b).  
  ...
  +eapply H_Consequence with (P' := wp <{while b do c end}> Q).
   -apply H_While. apply IHc. intros st st' H H'.
    destruct H'. unfold wp in *. intros. apply H0.
    ...
    *apply E_WhileFalse. destruct (beval st b). 
     exfalso. apply H0. reflexivity. reflexivity.
    *assumption.
Qed.
(** [] *)
