...
(** **** Exercise: 1 star, standard, optional (fact_steps)
  ...
  --> ST_AppAbs
    test 1=0 then 1 else 1 * (fix F (pred 1))
  --> ST_Test0_Nonzero
    1 * (fix F (pred 1))
  --> ST_FixAbs + ST_Mult2
    1 * ((\x. test x=0 then 1 else x * (fix F (pred x))) (pred 1))
  --> ST_PredNat + ST_Mult2 + ST_App2
    1 * ((\x. test x=0 then 1 else x * (fix F (pred x))) 0)
  --> ST_AppAbs + 3 x ST_Mult2
    1 * (test 0=0 then 1 else 0 * (fix F (pred 0)))
...
Fixpoint subst (x : string) (s : tm) (t : tm) : tm :=
  ...
  | <{t.snd}> => <{([x:=s] t).snd}>
  (* let *)
  | <{let y = t1 in t2}> =>  <{let y = ([x:=s] t1) in ( {if String.eqb x y then t2 else <{[x:=s] t2}>} ) }>
  (* fix *)
  | <{fix t}> => <{fix ([x:=s] t)}>
  end
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Snd : forall Gamma t T1 T2,
      ...
      Gamma |-- (t.snd) \in T2
  (* let *)
  | T_Let : forall Gamma t1 t2 T1 T2 x,
      Gamma |-- t1 \in T1 ->
      (x|->T1;Gamma)|-- t2 \in T2 ->
      Gamma |-- (let x=t1 in t2) \in T2
  (* fix *)
...
Example typechecks :
  empty |-- tm_test \in Nat.
Proof. unfold tm_test. eauto 20. Qed.
...
Proof with eauto.
  ...
  -(*T_fst*)
    right. destruct IHHt...
    +destruct H; try solve_by_invert. exists v1...
    +destruct H. exists <{x0.fst}>...
  -(*T_snd*)
    right. destruct IHHt...
    +destruct H; try solve_by_invert. exists v2...
    +destruct H. exists <{x0.snd}>...
  -(*T_let*)
   destruct IHHt1... right.  destruct H...
  -(*T_fix *)
   right. destruct IHHt...
   +destruct H;try solve_by_invert...
   +destruct H...
