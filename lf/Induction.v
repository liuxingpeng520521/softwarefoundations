...
Theorem mul_0_r : forall n:nat,
  ...
  Proof.
    intros n. induction n as [| n' IHn'].
    -simpl. reflexivity.
    -simpl. rewrite -> IHn'.   (* 当 n = S n' 时，使用归纳假设并化简 *)
    reflexivity.
  Qed.
...
Theorem add_comm : forall n m : nat,
  ...
  Proof.
    intros n m.                (* 引入变量 *)
    induction n as [| n' IHn'].   (* 对 n 进行归纳 *)
    - rewrite <- plus_n_O.      (* 当 n = 0 时，使用加法的单位元素性质 *)
      reflexivity.
    - simpl. rewrite -> IHn'. rewrite -> plus_n_Sm.  (* 当 n = S n' 时，应用归纳假设 *)
      reflexivity.
  Qed.
...
Theorem even_S : forall n : nat,
  ...
  Proof.
    intros n. induction n as [|n' IHn'].
    -simpl. reflexivity.
    -rewrite -> IHn'.   (* 然后简化及应用归纳假设 *)
    rewrite -> negb_involutive. 
    simpl.   (* 最后使用 negb 的自反性质完成证明 *)
    reflexivity.
    Qed.
...
Theorem n_mult_1_plus_k: forall m n:nat, m*(S n) = m + m * n.
    Proof.
      intros m n.
...
Theorem mul_comm : forall m n : nat,
  m * n = n * m.
Proof.
intros m n. induction n as [|  m' IHm'].
-simpl. rewrite <- mult_n_O. reflexivity.
-simpl. rewrite->n_mult_1_plus_k. rewrite->IHm'. reflexivity.
Qed.
...
Theorem andb_false_r : forall b : bool,
  ...
  Proof.
    intros b.         (* 引入变量 *)
    destruct b.       (* 对 b 进行分类讨论：当 b = true 和当 b = false *)
    - simpl. reflexivity.      (* 当 b = true 时，andb b false 化简为 false *)
    - simpl. reflexivity.      (* 当 b = false 时，andb b false 化简为 false *)
  Qed.
...
Proof.
  intros n. simpl. rewrite->add_comm. simpl. reflexivity.
Qed.
...
Theorem bin_to_nat_pres_incr : forall b : bin,
  ...
  Proof.
    ...
    -simpl. reflexivity.
    -simpl. reflexivity.
    -simpl. rewrite->IHB''. simpl. rewrite->add_comm. simpl. reflexivity. 
  Qed.
(** [] *)
...
Proof.
  ...
  -simpl. reflexivity.
  -simpl. rewrite->bin_to_nat_pres_incr. rewrite->IHb'. reflexivity.
Qed.
(** [] *)
...
Lemma nat_to_bin_doubleA : forall n : nat,
  nat_to_bin (n+n) =double_bin (nat_to_bin(n)).
Proof.
  intros n. induction n.
  -simpl.  reflexivity.
  -simpl. rewrite->add_comm. simpl. rewrite double_incr_bin.  rewrite->IHn. simpl. reflexivity.
Qed.
...
Proof.
  ...
  induction b.
  -simpl. reflexivity.
  -simpl. rewrite->nat_to_bin_doubleA. rewrite IHb. reflexivity.
  -simpl. rewrite->nat_to_bin_doubleA. rewrite IHb. reflexivity.
Qed.
