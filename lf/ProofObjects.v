...
Proof. exists 4. apply ev_SS. apply ev_SS. apply ev_0. Qed.
Print some_nat_is_even'.*)
(** **** Exercise: 2 stars, standard (ex_ev_Sn)

    Construct a proof object for the following proposition. *)
(*Theorem ex_ev_Sn' : ex (fun n => ev (S n)).
Proof. exists 3. apply ev_SS. apply ev_SS. apply ev_0. Qed.
Print ex_ev_Sn'.*)
Definition ex_ev_Sn : ex (fun n => ev (S n)) :=
  ex_intro (fun n : nat => ev (S n)) 3 (ev_SS 2 (ev_SS 0 ev_0)).
(** [] *)
...
Definition and_assoc : forall P Q R : Prop,
    ...
    fun (P Q R : Prop) H=>
    match H with
    |conj M N => match N with 
                |conj B V=>conj (conj M B) V
                 end
    end.
(** [] *)
...
Definition or_distributes_over_and : forall P Q R : Prop,
    ...
    fun (P Q R : Prop)=>conj
     (fun H =>
    match H with
    |or_introl M  => conj (or_introl M) (or_introl M)
    |or_intror N => match N with
                   |conj B V=>conj (or_intror B) (or_intror V)
                    end
    end)
     (fun H=>
    match H with
    |conj M N=> match M with
              |or_introl B  => or_introl B
              |or_intror X => match N with
                              |or_introl C  => or_introl C
                              |or_intror D => or_intror (conj X D)
              end
              end
    end).
...
Definition double_neg : forall P : Prop,
    P -> ~~P:=fun (P : Prop) (H : P) => (fun H0 : ~ P => H0 H).
...
Definition contradiction_implies_anything : forall P Q : Prop,
    ...
    fun (P Q : Prop) (H : P /\ ~ P) =>
    match H with
    | conj x x0 => False_ind Q (x0 x)
    end.
Definition de_morgan_not_or : forall P Q : Prop,
    ~ (P \/ Q) -> ~P /\ ~Q:=
    fun (P Q : Prop) (H : ~ (P \/ Q)) =>
    conj ((fun H0 : P => H (or_introl H0)))
      ((fun H0 : Q => H (or_intror H0))).
(** **** Exercise: 2 stars, standard (currying) *)
Definition curry : forall P Q R : Prop,
    ((P /\ Q) -> R) -> (P -> (Q -> R)):=
    fun (P Q R : Prop) (H : P /\ Q -> R) (H0 : P) (H1 : Q) => H (conj H0 H1).

Definition uncurry : forall P Q R : Prop,
    (P -> (Q -> R)) -> ((P /\ Q) -> R):=
    fun (P Q R : Prop) (H : P -> (Q -> R)) (H0 : (P /\ Q))=>
    match H0 with
    |conj M N => H M N
    end.
(** [] *)
...
Proof.   intros. apply H. split;intros.
+auto.
+apply I.
Qed.
...
Proof. unfold  proof_irrelevance. intros.
assert(True = P). { apply pe_implies_true_eq. apply H. apply pf1. }
generalize dependent pf1. generalize dependent pf2.
rewrite<-H0. intros. destruct pf1. destruct pf2. reflexivity.
Qed. 
(* 2023-03-25 11:11 *)
